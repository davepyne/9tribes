...
- [Combat Lethality Investigation](combatLethalityInvestigation.md) — 5 interventions ALL failed to increase kills; snowball feedback absorbs per-combat tuning. Update: 3/10 decisive games after unlockedRecipeIds fix.
...
- [Progression Pipeline Fix](progressionPipelineFix.md) — 3-cycle investigation fixed progression. decisiveGames now 3/10 after unlockedRecipeIds fix. Frost Wardens dominate (3 wins).
...
- [ClientState Extension Pattern](clientStateExtensionPattern.md) — pipeline for new game-state UI panels: type to builder in inspectors/ to GameController.getPlayState() to component reads state.xxx
...
- [Dead Field unlockedRecipeIds](deadFieldUnlockedRecipeIds.md) — initialized to [] but never written to; broke strategic AI hybrid goals and harness reporting. Pattern: verify write path exists for any new tracking field.
...
- [Skirmish Pursuit Domain](skirmishPursuitDomain.md) — hitrun redesigned from dead retreat to +2 pursuit damage when winning exchange; full T1 to T3 progression chain documented
