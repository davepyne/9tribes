...
import type { ChassisId, ComponentId } from '../types.js';
...
import {
  ...
  meetsLearnedDomainRequirement,
} from '../systems/domainProgression.js';
...
export interface PrototypeValidationOptions {
  ignoreResearchRequirements?: boolean;
  ignoreProgressionRequirements?: boolean;
}
...
export function validatePrototype(
  ...
  const seenSlotTypes = new Map<string, ComponentId>();
  ...
  for (const componentId of componentIds) {
    ...
    if (!component.compatibleChassis.includes(chassisId)) {
      errors.push(
        `Component '${componentId}' (${component.slotType}) is not compatible with chassis '${chassisId}'`
      );
    }
    ...
    if (seenSlotTypes.has(component.slotType)) {
      errors.push(
        `Duplicate slot type '${component.slotType}' - can only have one ${component.slotType} component`
      );
    } else {
