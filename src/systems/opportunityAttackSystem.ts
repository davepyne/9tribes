 ...
 */
function canDisengageFree(unit: Unit, state: GameState): boolean {
 ...
 */
function getWeaponTags(proto: Prototype, registry: RulesRegistry): string[] {
  const tags: string[] = [];
 ...
 */
export function applyOpportunityAttacks(
  gameState: GameState,
  movingUnitId: UnitId,
  originHex: HexCoord,
  targetHex: HexCoord,
  ...
  for (const neighborHex of getNeighbors(originHex)) {
    ...
    if (stillAdjacentKeys.has(hexToKey(neighborHex))) continue;

    const opportunistId = getUnitAtHex(current, neighborHex);
    if (!opportunistId) continue;

    const opportunist = current.units.get(opportunistId);
    if (!opportunist) continue;

    // Must be an enemy, alive, and not routed.
    if (opportunist.factionId === movingUnit.factionId) continue;
    if (opportunist.hp <= 0 || opportunist.routed) continue;

    // Only melee-role units can snap-attack a disengaging unit.
    const oppProto = current.prototypes.get(opportunist.prototypeId);
    if (!oppProto || oppProto.derivedStats.role !== 'melee') continue;
    ...
    if (!targetUnit || targetUnit.hp <= 0) break; // destroyed by a prior OA in this loop
