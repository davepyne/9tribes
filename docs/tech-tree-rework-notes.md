...
> Status: Phase 1 (creative content + source-of-truth migration + bug fixes) — complete. Phase 2 (Tier 1 wiring) — complete. Tier 2 wiring — complete. Tier 3 wiring — complete.
...
- **Hitrun T2 Bloodtrail momentum**: ✅ **wired.** `bloodtrailMomentumEnabled` in `capabilityDoctrine.ts`; `woundsReceivedThisTurn` on Unit incremented in `apply.ts` whenever a unit takes damage (both attacker and defender). Consumed in `factionTurnEffects.ts` per-unit refresh loop: each wound grants +1 movement next turn, capped at `maxMoves+3`. Reset in both `factionTurnEffects.ts` and `turnSystem.ts`. 12 tests in `tests/bloodtrailMomentum.test.ts`.
...
- **Charge T3 splash + chain amplification**: ✅ **wired.** `chargeSplashEnabled` / `chargeChainEnabled` in `capabilityDoctrine.ts`. Splash: `apply.ts` deals 50% of charge damage (min 1) to all enemy units adjacent to the defender. Chain: `preview.ts` traces the charge path via new `hexLineAwayFrom()` in `grid.ts`, counts friendly units along it, grants +10% situational attack modifier per ally (capped at 5 = +50%). Both gated behind native charge T3. Combat effects logged for splash, chain, and chain bonus. Shared `applyPoisonInRange()` helper extracted to DRY poison-splash across mycelium network, spore-jump, and lethal ambush. Tests in `tests/chargeChainAndSplash.test.ts`; shared test helpers extracted to `tests/helpers/combatSetup.ts`.
...
- **Camel native T3 — Oasis declaration**: ✅ **wired.** `canDeclareOasis` in `capabilityDoctrine.ts`, gated on `hasNativeT3('camel_adaptation')`. New module `src/systems/oasisSystem.ts` exports `declareOasis(state, factionId, centerHex)` which validates doctrine + land terrain, calls `setTerrainInRadius(state, hex, 2, 'desert')`, increments `faction.oasisDeclared`. Player verb wired via `'declare_oasis'` ClientAction (CommandTray "Proclaim Oasis" button → GameController → GameSession → sessionUtils → oasisSystem). AI heuristic in `src/systems/unit-activation/oasis.ts` (`getOasisOpportunity`, score threshold 8, requires 2+ friendly units in radius). Once-per-game cap via `faction.oasisDeclared`. 13 tests in `tests/oasisDeclaration.test.ts`.
- **River native T3 — Submerge**: ✅ **wired.** `submergeEnabled` in `capabilityDoctrine.ts`, gated on `hasNativeT3('river_stealth')`. New module `src/systems/submergeSystem.ts` exports `getConnectedWaterway(state, origin, maxRange?)` (BFS flood-fill through `WATER_TERRAINS`, capped at 8 hexes, excluding occupied destinations), `canSubmerge(state, factionId, unitId)`, and `executeSubmerge(state, factionId, unitId, destination)` (teleports unit, enters stealth, sets spent). Player verb wired via `'submerge'` ClientAction — CommandTray "Submerge" button sets targeting mode, right-click on highlighted waterway hex dispatches the teleport. AI heuristic in `src/systems/unit-activation/submerge.ts` (`getSubmergeOpportunity`, score threshold 6, prefers destinations near enemies for ambush). 15 tests in `tests/submergeSystem.test.ts`.
...
- **Nature native T3 — Sapling creation**: ✅ **wired.** `saplingOnKillEnabled` in `capabilityDoctrine.ts`, gated on `hasNativeT3('nature_healing')`. Post-kill block in `apply.ts` after spore-jump: calls `setTerrainAt(current, defender.position, 'forest')` from `terrainMutationSystem.ts` on every kill. Attacker gains +1 max HP (and +1 current HP) per sapling kill, capped at +3 lifetime via history entries (`getHistoryByType(attacker, 'sapling_kill').length`). Combat effect label "Sapling" with aftermath category. Resolution fields `saplingApplied` and `saplingMaxHpBonus` for UI feedback. 8 tests in `tests/saplingCreation.test.ts`.
- **Nature T2 — Wounded Earth**: ✅ **wired.** `woundedEarthEnabled` / `woundedEarthHealEnabled` in `capabilityDoctrine.ts`. Foreign: terrain absorbs 25% (rounded down) of damage on forest/jungle, reducing defender HP loss; `terrainDamageAbsorption` field on Unit for UI feedback. Native (Druidic Circle): defender takes full damage but adjacent allies on forest/jungle heal for the absorbed amount. Can save a defender from death via absorption (updates `defenderActuallyDestroyed`). Combat effect label for the combat log. 17 tests in `tests/woundedEarth.test.ts`.
