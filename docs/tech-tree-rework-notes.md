...
The damage consumer hook now lives in `simulation/environmentalEffects.ts:187-194` (added by the Toxic Bloom wiring) and applies to any ZoneEffect with `damagePerTurn > 0` — Maelstrom will reuse it as-is. The movement-penalty consumer is still pending, to be wired alongside Maelstrom.
