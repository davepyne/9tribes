...
import { CY_OFFSET, TEXTURES } from '../assets/keys';
...
export class SelectionRenderer {
  ...
  render(world: WorldViewModel, inspectedKey: string | null, hoveredKey: string | null) {
    this.layer.removeAll(true);

    if (hoveredKey) {
      const point = this.worldToScreen(...hoveredKey.split(',').map(Number) as [number, number]);
      this.layer.add(
        this.scene.add.image(point.x, point.y + CY_OFFSET, TEXTURES.selection, 1)
          .setOrigin(0.5, 0.5)
          .setAlpha(0.45)
          .setTint(0xf7e7bf),
      );
    ...
    if (inspectedKey) {
      ...
      this.layer.add(
        this.scene.add.image(point.x, point.y + CY_OFFSET, TEXTURES.selection, 0)
          .setOrigin(0.5, 0.5)
          .setScale(1.1)
          .setAlpha(0.9)
          .setTint(0xffd84d),
