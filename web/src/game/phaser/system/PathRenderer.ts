...
export class PathRenderer {
  ...
  render(world: WorldViewModel) {
    ...
    for (const hex of world.overlays.disembarkHexes ?? []) {
      const point = this.worldToScreen(hex.q, hex.r);
      const ring = this.scene.add.ellipse(point.x, point.y - 8, 62, 28, 0x3a8fd6, 0.26)
        .setStrokeStyle(3, 0x9ec8f7, 0.95);
      this.layer.add(ring);
    }
    ...
    // Render queued path (cyan dashed) — before hover preview so hover draws on top
    if (world.overlays.queuedPath.length >= 2) {
      const qpGraphics = this.scene.add.graphics();
      qpGraphics.lineStyle(4, 0x4ecdc4, 0.65);
      for (let index = 0; index < world.overlays.queuedPath.length - 1; index += 1) {
        ...
        this.drawDashedLine(qpGraphics, from.x, from.y - 8, to.x, to.y - 8, 8, 5);
      }
      this.layer.add(qpGraphics);
    ...
    // Hover path preview (gold solid) — draws on top of queued path
  ...
  private drawDashedLine(
    ...
    let drawing = true;
    while (pos < dist) {
      const segLen = drawing ? Math.min(dashLength, dist - pos) : Math.min(gapLength, dist - pos);
      const sx = x1 + nx * pos;
      const sy = y1 + ny * pos;
