    ...
    <div className="tribe-list">
      {helpContent.tribes.map((tribe) => (
        <article
          ...
          style={{ '--tribe-accent': tribe.color } as React.CSSProperties}
        >
          {/* Header */}
          <div className="tribe-card__header">
            <span
              className="tribe-card__swatch"
              style={{ backgroundColor: tribe.color }}
            />
            <span className="tribe-card__name">{tribe.name}</span>
            <span
              className="tribe-card__domain"
              style={{ color: domainColor(tribe.nativeDomain) }}
              title={domainDisplayName(tribe.nativeDomain)}
            >
