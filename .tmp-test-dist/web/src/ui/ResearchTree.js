...
const TIERS = [1, 2, 3];
export function ResearchTree({ nodes, selectedNodeId, onSelectNode }) {
    ...
    if (unlockedDomains.length === 0) {
        return (<div className="research-tree-empty">
    ...
    return (<div className="research-tree-shell">
      <div className="research-tree__domains">
        {unlockedDomains.map((domain) => {
            ...
            return (<div key={domain.id} className={`research-domain-row${isNative ? ' research-domain-row--native' : ''}`}>
              ...
              <div className="research-domain-row__nodes">
                {TIERS.map((tier) => {
                    ...
                    if (!node)
                        return null;
                    return (<div key={node.nodeId} className="research-domain-row__cell">
                      {tier < 3 && (<div className="research-domain-row__connector" aria-hidden="true"/>)}
                      <ResearchNode node={node} selected={node.nodeId === selectedNodeId} onSelect={() => onSelectNode(node.nodeId)}/>
                    </div>);
                })}
              </div>
            </div>);
