...
export function domainGlyph(domainId) {
...
export function domainDisplayName(domainId) {
    return DOMAIN_NAMES[domainId] ?? domainId.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
}
function resolveSynergies(allDomains, pairEligibleDomains, emergentEligibleDomains, nativeDomain) {
    const pairs = pairSynergiesData.pairSynergies;
    const rules = emergentRulesData.rules;
...
function computeRuleProgress(rule, learnedDomains) {
    const satisfiedCategories = [];
    const satisfiedDomains = [];
    const missingDomains = [];
    if (rule.domainSets) {
        const alreadyCredited = new Set();
        for (const [category, domainList] of Object.entries(rule.domainSets)) {
            const matched = domainList.find((d) => learnedDomains.includes(d) && !alreadyCredited.has(d));
            if (matched) {
    ...
    if (rule.mobilityDomains) {
        ...
        if (count >= 3)
            satisfiedCategories.push('mobility');
        for (const d of rule.mobilityDomains) {
            if (learnedDomains.includes(d))
                satisfiedDomains.push(d);
            else
    ...
    if (rule.combatDomains) {
        ...
        if (count >= 3)
            satisfiedCategories.push('combat');
    ...
    const isComplete = (totalCategories > 0 && satisfiedCategories.length >= totalCategories) ||
...
// ── Sub-components ──
function DomainDot({ domainId, size = 16, isNative = false, }) {
    const color = domainColor(domainId);
    const glyph = domainGlyph(domainId);
    return (<span className="syn-dot" style={{
            '--syn-dot-color': color,
            '--syn-dot-size': `${size}px`,
        }} title={domainDisplayName(domainId)} data-native={isNative || undefined}>
      <span className="syn-dot__glyph">{glyph}</span>
    </span>);
}
function PairConnection({ pair, domainIndexMap, }) {
    ...
    const y = dotSize / 2;
    return (<line x1={xA} y1={y} x2={xB} y2={y} className="syn-pair-line" style={{ stroke: domainColor(pair.domains[0]) }}/>);
}
export function SynergyChip({ state }) {
    ...
    const learnedDomains = useMemo(() => {
        if (factionLearnedDomains.length > 0)
            return factionLearnedDomains;
        // Fallback to native-only when nothing else learned yet
        return [nativeDomain];
    }, [factionLearnedDomains, nativeDomain]);
    const pairEligibleDomains = useMemo(() => capabilities.filter((cap) => cap.level >= 3).map((cap) => cap.domainId), [capabilities]);
    const emergentEligibleDomains = useMemo(() => capabilities.filter((cap) => cap.level >= 2).map((cap) => cap.domainId), [capabilities]);
    const resolved = useMemo(() => resolveSynergies(learnedDomains, pairEligibleDomains, emergentEligibleDomains, nativeDomain), [learnedDomains, pairEligibleDomains, emergentEligibleDomains, nativeDomain]);
    const handleClick = useCallback((e) => {
        e.stopPropagation();
        setExpanded((v) => !v);
    }, []);
    const handleClose = useCallback(() => setExpanded(false), []);
    const handlePanelClick = useCallback((e) => e.stopPropagation(), []);
    const hasContent = resolved.foreignDomains.length > 0 || resolved.activePairs.length > 0;
    return (<div className="syn-chip-wrap" onClick={handleClick}>
      {/* ── Compact Chip ── */}
      <button type="button" className={`syn-chip ${hasContent ? 'syn-chip--active' : ''} ${expanded ? 'syn-chip--open' : ''}`} style={{ '--syn-accent': factionColor }} title="Ability Synergies — click to expand">
        <span className="syn-chip__domains">
          <DomainDot domainId={nativeDomain} size={14} isNative/>
          {resolved.foreignDomains.slice(0, 3).map((d) => (<DomainDot key={d} domainId={d} size={14}/>))}
          {resolved.foreignDomains.length > 3 && (<span className="syn-chip__more">+{resolved.foreignDomains.length - 3}</span>)}
        </span>

        {resolved.activePairs.length > 0 && (<span className="syn-chip__pairs">
        ...
        {resolved.emergentProgress.some((ep) => ep.progress > 0 && !ep.isComplete) && (() => {
            ...
            return best ? (<span className="syn-chip__emergent" title={`${best.rule.name}: ${best.satisfiedCategories.length}/${best.totalCategories}`}>
        ...
        {resolved.activeTriple && (<span className="syn-chip__triple" style={{ color: factionColor }}>
          ...
          <div className="syn-panel" onClick={handlePanelClick}>
            ...
            <section className="syn-section">
              ...
              <div className="syn-constellation">
                <svg className="syn-constellation__svg" width="100%" height="36" preserveAspectRatio="xMinYMid meet">
                  {resolved.activePairs.map((pair) => (<PairConnection key={pair.pair.id} pair={pair} domainIndexMap={Object.fromEntries(resolved.allDomains.map((d, i) => [d, i]))}/>))}
                </svg>
                <div className="syn-constellation__dots">
                  {resolved.allDomains.map((d) => (<DomainDot key={d} domainId={d} size={24} isNative={d === nativeDomain}/>))}
                </div>
              </div>
              <div className="syn-domain-list">
                {resolved.allDomains.map((d) => {
                const isNative = d === nativeDomain;
                return (<div key={d} className="syn-domain-item" data-native={isNative || undefined}>
                      <DomainDot domainId={d} size={18} isNative={isNative}/>
                      <span className="syn-domain-item__name">{domainDisplayName(d)}</span>
                      {isNative && (<span className="syn-domain-item__tag syn-domain-item__tag--native">Native</span>)}
                      {!isNative && (<span className="syn-domain-item__tag syn-domain-item__tag--codified">Acquired</span>)}
                    </div>);
            })}
              </div>
              {resolved.allDomains.length <= 1 && (<p className="syn-empty">No foreign domains unlocked yet. Defeat enemies, then sacrifice carriers at your capital.</p>)}
            </section>
            ...
            <section className="syn-section">
              <h4 className="syn-section__label">
                Active Pairs
                {resolved.activePairs.length > 0 && (<span className="syn-section__count">{resolved.activePairs.length}</span>)}
              </h4>
              {resolved.activePairs.length > 0 ? (<ul className="syn-pair-list">
                  {resolved.activePairs.map(({ pair, domains }) => (<li key={pair.id} className="syn-pair-item">
            ...
            <section className="syn-section">
              ...
              {resolved.activeTriple ? (<div className="syn-triple-active">
              ...
              <div className="syn-rule-list">
                ...
                .map((ep) => (<div key={ep.rule.id} className={`syn-rule-item ${ep.isComplete ? 'syn-rule-item--done' : ''}`}>
                      ...
                      <div className="syn-rule-item__bar">
                        <span className="syn-rule-item__fill" style={{
                    ...
                    '--syn-rule-color': ep.isComplete ? factionColor : domainColor(ep.satisfiedDomains[0] ?? ''),
                      ...
                      <div className="syn-rule-item__detail">
                        {ep.isComplete ? (<span className="syn-rule-item__effect">{ep.rule.effect.description}</span>) : (<>
                            <span>Have: </span>
                            {ep.satisfiedDomains.map((d) => (<span key={d} className="syn-rule-item__domain" style={{ color: domainColor(d) }}>
                                {domainGlyph(d)} {domainDisplayName(d)}
                              </span>))}
                            {ep.missingDomains.length > 0 && (<>
                                <span> &middot; Need: </span>
                                {ep.missingDomains.slice(0, 2).map((d) => (<span key={d} className="syn-rule-item__domain syn-rule-item__domain--missing">
                                    {domainGlyph(d)} {domainDisplayName(d)}
                                  </span>))}
                              </>)}
                          </>)}
