...
import { SynergyChip } from './SynergyChip.js';
const gameMenu = [
...
export function GameMenuBar({ state, onOpenResearch, onOpenHelp, onRestartSession, onMenuAction }) {
    ...
    return (<nav className="gmb-root" style={{ '--gmb-faction-color': factionColor }}>
      ...
      <div className="gmb-status">
        ...
        {researchChip ? (<button type="button" className="gmb-chip gmb-chip--research" onClick={onOpenResearch}>
            <span className="gmb-chip-label">Research</span>
            <span>{researchChip.activeNodeName ?? 'Idle'}</span>
          </button>) : null}

        {state.hud.supply ? (<div className={`gmb-chip gmb-chip--supply${state.hud.supply.deficit > 0 ? ' gmb-chip--deficit' : ''}`} title={state.hud.supply.deficit > 0
