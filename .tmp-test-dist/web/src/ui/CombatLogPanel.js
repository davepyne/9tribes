...
function buildModifierEntries(event) {
    const entries = [];
...
export function CombatLogPanel({ events, isOpen, onToggle }) {
    ...
    return (<div className={`clp-root${isOpen ? ' clp-root--open' : ''}`}>
      {/* Toggle tab */}
      <button type="button" className="clp-toggle" onClick={() => onToggle()} aria-label={isOpen ? 'Close combat log' : 'Open combat log'}>
      ...
      <div className="clp-body">
        <div className="clp-header">
          ...
          <button type="button" className="clp-clear" onClick={handleClear} disabled={localEvents.length === 0}>
            Clear
          </button>
        </div>

        <div className="clp-scroll" ref={scrollRef}>
          {localEvents.length === 0 ? (<p className="clp-empty">No combat recorded yet.</p>) : (localEvents.map((event, idx) => (<CombatEntry key={`${event.attackerUnitId}-${event.defenderUnitId}-${event.round}-${idx}`} event={event}/>)))}
        </div>
      </div>
    </div>);
}
function CombatEntry({ event }) {
    const modifiers = buildModifierEntries(event);
    const outcome = formatOutcome(event);
    return (<div className="clp-entry">
      {/* Combat header */}
      ...
      {/* Modifier breakdown */}
      {modifiers.length > 0 && (<div className="clp-modifiers">
          {modifiers.map((mod, i) => (<div key={`${mod.label}-${i}`} className={`clp-modifier clp-modifier--${mod.kind}`}>
      ...
      {/* Outcome */}
      {outcome && (<div className="clp-outcome">{outcome}</div>)}
