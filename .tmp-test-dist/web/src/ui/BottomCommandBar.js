export function BottomCommandBar({ state, timelineMax, onSetTurn, onEndTurn, onRestartSession }) {
    ...
    const spentLabel = selectedUnit
        ...
        : null;
    return (<section className="bottom-command-bar">
      <div className="panel command-panel">
        ...
        {state.mode === 'play' ? (<>
            <div className="timeline-controls">
              ...
              {onRestartSession ? (<button type="button" onClick={() => {
            ...
            {selectedUnit ? (<div className="event-list">
              ...
              </div>) : (<p className="quiet-copy">Select a friendly unit, then drag it to a highlighted tile to move. Press A to target an attack.</p>)}
          </>) : (<>
            <div className="timeline-controls">
            ...
            <input className="timeline-slider" type="range" min={0} max={timelineMax} value={state.turnIndex} onChange={(event) => onSetTurn(Number(event.target.value))}/>
          </>)}
      </div>
      ...
      <div className="panel command-panel">
        ...
        <div className="event-list">
          {state.debug.turnEvents.length === 0 ? (<p className="quiet-copy">{state.mode === 'play' ? 'No session events recorded yet.' : 'No debug events recorded for this round.'}</p>) : (state.debug.turnEvents.map((event, index) => (<div className="event-row" key={`${event.sequence ?? event.round}-${index}`}>
