...
function emergentConditionLabel(rule) {
...
export function SynergyEncyclopediaTab() {
    ...
    const filteredSynergies = useMemo(() => {
        ...
        return pairSynergiesData.pairSynergies.filter((pair) => {
            // Domain filter
            if (activeFilters.size > 0) {
                const domains = pair.domains;
                for (const f of activeFilters) {
                    if (!domains.includes(f))
                        return false;
                }
            }
            // Search filter
            if (search) {
                ...
                const tagMatch = pair.requiredTags.some((t) => t.toLowerCase().includes(search));
    ...
    const emergentRules = useMemo(() => emergentRulesData.rules.filter((r) => r.condition !== 'default'), []);
    return (<div className="syn-enc">
      {/* Search */}
      <input type="text" className="syn-enc__search" placeholder="Search synergies by name, tag, or description…" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>

      {/* Domain filter row */}
      <div className="syn-enc__filters">
        {activeFilters.size > 0 && (<button type="button" className="syn-enc__filter-clear" onClick={clearFilters}>
            All
          </button>)}
        {ALL_DOMAIN_IDS.map((domainId) => {
            const isActive = activeFilters.has(domainId);
            return (<button key={domainId} type="button" className={`syn-enc__filter-dot${isActive ? ' syn-enc__filter-dot--active' : ''}`} style={{ '--dot-color': domainColor(domainId) }} onClick={() => toggleFilter(domainId)} title={domainDisplayName(domainId)}>
      ...
      {/* Synergy list */}
      {filteredSynergies.length > 0 ? (<div className="syn-enc__list">
          {filteredSynergies.map((pair) => {
                const domains = pair.domains;
                const description = guideMap.get(pair.id) ?? pair.description;
                const tags = pair.requiredTags.filter((t, i, arr) => arr.indexOf(t) === i);
                return (<div key={pair.id} className="syn-enc__item">
                <div className="syn-enc__item__domains">
                  <span className="syn-enc__item__dot" style={{ backgroundColor: domainColor(domains[0]) }} title={domainDisplayName(domains[0])}>
                    {domainGlyph(domains[0])}
                  </span>
                  <span className="syn-enc__item__plus">+</span>
                  <span className="syn-enc__item__dot" style={{ backgroundColor: domainColor(domains[1]) }} title={domainDisplayName(domains[1])}>
                    {domainGlyph(domains[1])}
                  </span>
                </div>
                <div className="syn-enc__item__name">{pair.name}</div>
                <div className="syn-enc__item__desc">{description}</div>
                {tags.length > 0 && (<div className="syn-enc__item__tags">
                    {tags.map((tag) => (<span key={tag} className="syn-enc__item__tag">{tag}</span>))}
                  </div>)}
              </div>);
            })}
        </div>) : (<div className="syn-enc__empty">No synergies match your filters.</div>)}

      {/* Emergent Triple Stacks */}
      <div className="syn-enc__section">Emergent Triple Stacks</div>
      <div className="syn-enc__list">
        {emergentRules.map((rule) => (<div key={rule.id} className="syn-enc__emergent">
