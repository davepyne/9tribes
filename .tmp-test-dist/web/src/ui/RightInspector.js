...
export function RightInspector({ state, onSetCityProduction }) {
    ...
    return (<aside className="right-inspector">
      ...
      {selectedCity ? (<>
          ...
          <section className="panel city-panel">
            ...
            {selectedCity.production.current ? (<div className="city-card">
              ...
              </div>) : (<p className="quiet-copy">No production is active in this city.</p>)}
            {selectedCity.production.queue.length > 0 ? (<div className="city-stack">
                {selectedCity.production.queue.map((item) => (<div className="city-list-row" key={`${item.type}-${item.id}`}>
                    <span>{item.name}</span>
                    <strong>{item.costLabel}</strong>
                  </div>))}
              </div>) : (<p className="quiet-copy">No queued items.</p>)}
          </section>
          ...
          <section className="panel city-panel">
            ...
            {!selectedCity.canManageProduction ? (<p className="quiet-copy">
            ...
            <div className="city-stack">
              {selectedCity.productionOptions.map((option) => (<button key={option.prototypeId} type="button" className="city-option" disabled={option.disabled} onClick={() => onSetCityProduction?.(selectedCity.cityId, option.prototypeId)}>
          ...
          <section className="panel city-panel">
            ...
            {selectedCity.captureRamp && selectedCity.captureRamp.rampMultiplier < 1 ? (<p className="quiet-copy" style={{ marginTop: '0.5rem', fontSize: '0.8rem' }}>
          ...
          <section className="panel city-panel">
            ...
            <div className="city-stack">
              {selectedCity.villageReadiness.checklist.map((item) => (<div className="city-list-row" key={item.key}>
      ...
      {settlementPreview ? (<section className="panel">
          <div className="panel-heading compact">
          ...
          {settlementPreview.blockedReason ? (<p className="quiet-copy">{settlementPreview.blockedReason}</p>) : null}
        </section>) : null}
      ...
      <section className="panel">
        ...
        {selectedUnit ? (<>
            ...
            {selectedUnit.xp != null ? (<div className="meta-row">
                <span>XP</span>
      ...
      <section className="panel">
        ...
        <div className="faction-stack">
          {state.hud.factionSummaries.map((faction) => (<div className="faction-row" key={faction.id}>
      ...
      {state.mode === 'replay' ? (<>
          <section className="panel">
            ...
            {state.hud.recentCombat.length === 0 ? (<p className="quiet-copy">No structured battles this round.</p>) : (state.hud.recentCombat.map((event, index) => (<div className="inspector-entry" key={`${event.attackerUnitId}-${event.defenderUnitId}-${index}`}>
          ...
          <section className="panel">
            ...
            {state.hud.recentIntents.length === 0 ? (<p className="quiet-copy">No AI intents recorded this round.</p>) : (state.hud.recentIntents.map((intent, index) => (<div className="inspector-entry" key={`${intent.unitId}-${index}`}>
