export function TopHud({ state, turnBanner, onOpenResearch }) {
    const activeFactionColor = state.world.factions.find((faction) => faction.id === state.activeFactionId)?.color ?? '#d6a34b';
    const recoveringCityCount = state.world.cities.filter((city) => city.factionId === state.activeFactionId && city.turnsSinceCapture !== undefined).length;
    return (<header className="top-hud">
      ...
      <div className="top-hud__stats">
        ...
        <div className="status-chip status-chip--active-faction" style={{ '--chip-color': activeFactionColor }}>
        ...
        {state.hud.researchChip ? (<div className="status-chip status-chip--research" role="button" tabIndex={0} onClick={onOpenResearch} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ')
            onOpenResearch?.(); }}>
            <span className="chip-label">Research</span>
            <strong>{state.hud.researchChip.activeNodeName ?? 'Idle'}</strong>
          </div>) : null}
        {state.hud.supply ? (<div className={`status-chip${state.hud.supply.deficit > 0 ? ' status-chip--deficit' : ''}`} title={(() => {
