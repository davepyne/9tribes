...
export function CommandTray({ state, timelineMax, onSetTurn, onEndTurn, onSetTargetingMode, onBuildFort, onBuildCity, onSacrifice }) {
    ...
    const canSacrifice = (() => {
        if (!selectedUnit)
            return false;
        if (!selectedUnit.learnedAbilities || selectedUnit.learnedAbilities.length === 0)
            return false;
        if (!selectedUnit.isActiveFaction)
            return false;
    ...
    const canBuildFort = (() => {
        ...
        if (!selectedUnit.isActiveFaction)
            return false;
        if (selectedUnit.movesRemaining !== selectedUnit.movesMax)
            return false;
        const faction = state.world.factions.find((f) => f.id === selectedUnit.factionId);
        if (!faction || faction.id !== 'hill_clan' || faction.nativeDomain !== 'fortress')
            return false;
        const canBuildFieldForts = state.research?.nodes.some((node) => node.nodeId === 'fortress_t2' && node.state === 'completed') ?? false;
        if (!canBuildFieldForts)
            return false;
        const hasFort = state.world.improvements.some((improvement) => improvement.q === selectedUnit.q && improvement.r === selectedUnit.r);
        if (hasFort)
            return false;
    ...
    const canBuildCity = (() => {
        if (!selectedUnit)
            return false;
        if (!selectedUnit.isActiveFaction || !selectedUnit.isSettler)
            return false;
        if (selectedUnit.movesRemaining !== selectedUnit.movesMax)
            return false;
    ...
    return (<section className="ct-root">
      {state.mode === 'play' ? (<>
          <div className="ct-segment ct-segment--info">
            {selectedUnit ? (<>
                ...
                {showSacrificeFeedback ? (<span className="ct-codify-feedback">
                    Sacrificed - unlocked: {sacrificeFeedback.domains.join(', ')}
                  </span>) : null}

                {selectedUnit.learnedAbilities && selectedUnit.learnedAbilities.length > 0 && !showSacrificeFeedback ? (<span className="ct-knowledge-hint">
                    Carries: {selectedUnit.learnedAbilities.map((d) => formatDomainName(d)).join(', ')}
                    {!canSacrifice ? ' \u2014 return to Home City to Sacrifice' : ''}
                  </span>) : null}
                {selectedUnit.isSettler && !settlementPreview ? (<span className="ct-knowledge-hint">Press 'b' to build a city</span>) : null}
                {settlementPreview ? (<span className="ct-knowledge-hint">
          ...
          <div className="ct-segment ct-segment--actions">
            {selectedUnit ? (<>
                <button type="button" className={`ct-mode-btn${state.actions.targetingMode === 'move' ? ' ct-mode-btn--active' : ''}`} onClick={() => onSetTargetingMode('move')}>
                  Move
                </button>
                <button type="button" className={`ct-mode-btn${state.actions.targetingMode === 'attack' ? ' ct-mode-btn--active' : ''}`} onClick={() => onSetTargetingMode('attack')}>
                  Attack
                </button>
                {canBuildFort ? (<button type="button" className="ct-mode-btn" onClick={() => onBuildFort?.(selectedUnitId)}>
                    Build Fort
                  </button>) : null}
                {canBuildCity ? (<button type="button" className="ct-mode-btn" onClick={() => onBuildCity?.(selectedUnitId)}>
                    Build City
                  </button>) : null}
                {canSacrifice ? (<button type="button" className="ct-btn-codify" onClick={() => onSacrifice(selectedUnitId)}>
                    Sacrifice
                  </button>) : null}
              </>) : null}
          </div>
          ...
          <div className="ct-segment ct-segment--end">
            <button type="button" className="ct-btn-end-turn" disabled={!state.actions.canEndTurn} onClick={onEndTurn}>
              End Turn [Enter]
            </button>
          </div>
        </>) : (<>
          ...
          <div className="ct-segment ct-segment--replay-slider">
            <input className="ct-timeline-slider" type="range" min={0} max={timelineMax} value={state.turnIndex} onChange={(e) => onSetTurn(Number(e.target.value))}/>
