...
import { getCombatSummary, getIntentSummary } from '../game/view-model/worldViewModel.js';
import abilityDomains from '../data/ability-domains.json' with { type: 'json' };
...
function getDomainDescription(domainId) {
    const domain = abilityDomains.domains[domainId];
    return domain?.baseEffect?.description;
}
export function ContextInspector({ state, isOpen, onOpen, onClose, onSetCityProduction, onCancelCityProduction, onRemoveFromQueue, onSetTargetingMode, onPrepareAbility, onBoardTransport, onDisembarkUnit, onDeselect, onCloseCityProduction }) {
    ...
    if (!isOpen || !selection) {
        return (<aside className="ci-root">
        {selection && !isOpen && (<button type="button" className="ci-toggle" onClick={() => onOpen()} aria-label="Open inspector">
    ...
    return (<aside className="ci-root ci-root--open">
      <div className="ci-scroll">
        ...
        {/* ── Unit Inspector ── */}
        {selectedUnit ? (<div className="ci-section">
            ...
            <div className="ci-unit-combat">
              ...
              <div className="ci-stat-grid">
                ...
                <div className="ci-stat-cell">
                  ...
                  {selectedUnit.effectiveDefense !== selectedUnit.defense && (<span className="ci-stat-sub">→ {selectedUnit.effectiveDefense}</span>)}
            ...
            <div className="ci-unit-details">
              ...
              {selectedUnit.veteranLevel ? (<div className="meta-row">
                  <span>Veterancy</span>
            ...
            {/* Domains */}
            {(selectedUnit.nativeDomain || (selectedUnit.learnedAbilities && selectedUnit.learnedAbilities.length > 0)) ? (<div className="ci-domains">
                <p className="panel-kicker">Domains</p>
                {selectedUnit.nativeDomain && (<div className="meta-row">
                ...
                {selectedUnit.learnedAbilities && selectedUnit.learnedAbilities.length > 0 ? (<>
                    ...
                    {selectedUnit.learnedAbilities.map((domainId) => (<div key={domainId} className="ci-learned-ability">
                        <span className="ci-knowledge__pip">{formatDomainName(domainId)}</span>
                        {getDomainDescription(domainId) && (<p className="ci-learned-ability__desc">{getDomainDescription(domainId)}</p>)}
                      </div>))}
            ...
            {(selectedUnit.isStealthed || selectedUnit.poisoned || selectedUnit.routed || selectedUnit.preparedAbility) ? (<div className="ci-conditions">
                <p className="panel-kicker">Conditions</p>
                {selectedUnit.isStealthed && (<div className="meta-row ci-condition ci-condition--stealth">
                    <span>Stealthed</span>
                    <strong>Hidden from enemy sight</strong>
                  </div>)}
                {selectedUnit.poisoned && (<div className="meta-row ci-condition ci-condition--poison">
                    <span>Poisoned</span>
                    <strong>Taking damage over time</strong>
                  </div>)}
                {selectedUnit.routed && (<div className="meta-row ci-condition ci-condition--routed">
                    <span>Routed</span>
                    <strong>Broken morale — unable to act</strong>
                  </div>)}
                {selectedUnit.preparedAbility && (<div className="meta-row ci-condition ci-condition--prepared">
                    <span>Prepared</span>
                    <strong>{selectedUnit.preparedAbility === 'brace' ? 'Bracing (counter-attack bonus)' : 'Ambush (first-strike bonus)'}</strong>
                  </div>)}
              </div>) : null}

            {settlementPreview ? (<div className="ci-conditions">
                ...
                {settlementPreview.traits.map((trait) => (<div className="meta-row" key={trait.key}>
            ...
            {state.mode === 'play' ? (<div className="ci-actions">
                <button type="button" className={`ci-action-btn${state.actions.targetingMode === 'move' ? ' ci-action-btn--active' : ''}`} onClick={() => onSetTargetingMode('move')}>
                  Move
                </button>
                <button type="button" className={`ci-action-btn${state.actions.targetingMode === 'attack' ? ' ci-action-btn--active' : ''}`} onClick={() => onSetTargetingMode('attack')}>
                  Attack
                </button>
                {selectedUnit.canBrace ? (<button type="button" className="ci-action-btn" onClick={() => onPrepareAbility(selectedUnit.id, 'brace')}>
                    Brace
                  </button>) : null}
                {selectedUnit.canAmbush ? (<button type="button" className="ci-action-btn" onClick={() => onPrepareAbility(selectedUnit.id, 'ambush')}>
                    Ambush
                  </button>) : null}
                {selectedUnit.boardableTransportIds?.map((transportId) => (<button key={transportId} type="button" className="ci-action-btn" onClick={() => onBoardTransport(selectedUnit.id, transportId)}>
                    Board
                  </button>))}
                {selectedUnit.transportId && selectedUnit.validDisembarkHexes?.map((hex) => (<button key={`${hex.q},${hex.r}`} type="button" className="ci-action-btn" onClick={() => onDisembarkUnit(selectedUnit.id, selectedUnit.transportId, hex)}>
                    Land {hex.q},{hex.r}
                  </button>))}
              </div>) : null}

            {!selectedUnit.canAct ? (<p className="quiet-copy">
        ...
        {/* ── City Inspector ── */}
        {selectedCity ? (<div className="ci-section">
            <div className="ci-tabs-wrapper">
              {tabsCanScrollLeft && (<button type="button" className="ci-tabs-arrow ci-tabs-arrow--left" aria-label="Scroll tabs left" onClick={scrollTabsLeft}>
                  ‹
                </button>)}
              <div className="ci-tabs" ref={tabsRef} role="tablist">
                {['overview', 'production'].map((tab) => (<button key={tab} type="button" role="tab" className={`ci-tab${cityTab === tab ? ' ci-tab--active' : ''}`} onClick={() => setCityTab(tab)}>
                    {tab.charAt(0).toUpperCase() + tab.slice(1)}
                  </button>))}
              </div>
              {tabsCanScrollRight && (<button type="button" className="ci-tabs-arrow ci-tabs-arrow--right" aria-label="Scroll tabs right" onClick={scrollTabsRight}>
                  ›
                </button>)}
            </div>

            {cityTab === 'overview' ? (<div className="ci-tab-content">
                <div className="meta-row">
                  <span>Faction</span>
                ...
                {selectedCity.siteBonuses.traits.map((trait) => (<div className="meta-row" key={trait.key}>
                    <span>{trait.label}</span>
                    <strong>{trait.active ? `${trait.effect} (${trait.count})` : 'None'}</strong>
                  </div>))}
              </div>) : null}

            {cityTab === 'production' ? (<div className="ci-tab-content ci-prod-tab">
                {/* ── Current Production ── */}
                {selectedCity.production.current ? (<div className="pq-current">
                    ...
                    <div className="pq-progress">
                      <div className="pq-progress__fill" style={{ width: `${Math.min(100, (selectedCity.production.current.progress / selectedCity.production.current.cost) * 100)}%` }}/>
                    </div>
                    ...
                    {selectedCity.canManageProduction && (<button type="button" className="pq-cancel-btn" onClick={() => onCancelCityProduction(selectedCity.cityId)}>
                        Cancel
                      </button>)}
                  </div>) : (<div className="pq-idle">
                    <span className="pq-idle__dot"/>
                    Idle — select a unit to begin training
                  </div>)}

                {/* ── Queue ── */}
                {selectedCity.production.queue.length > 0 && (<div className="pq-queue">
                    ...
                    {selectedCity.production.queue.map((item, index) => (<div className="pq-queue-item" key={`${item.type}-${item.id}-${index}`}>
                        ...
                        {selectedCity.canManageProduction && (<button type="button" className="pq-queue-item__remove" onClick={() => onRemoveFromQueue(selectedCity.cityId, index)} aria-label={`Remove ${item.name} from queue`}>
                            ×
                          </button>)}
                      </div>))}
                  </div>)}
                ...
                {!selectedCity.canManageProduction ? (<p className="pq-readonly-hint">
                ...
                <div className="pq-unit-list">
                  {selectedCity.productionOptions.map((option) => (<button key={option.prototypeId} type="button" className={`pq-unit-row${option.disabled ? ' pq-unit-row--disabled' : ''}`} disabled={option.disabled} onClick={() => onSetCityProduction(selectedCity.cityId, option.prototypeId)}>
        ...
        {selection.type === 'hex' ? (<div className="ci-section">
            <p className="ci-desc">{state.hud.selectedDescription}</p>
            {state.hud.selectedMeta.map((entry) => (<div className="meta-row" key={entry.label}>
        ...
        {/* ── Village Inspector ── */}
        {selection.type === 'village' ? (<div className="ci-section">
            <p className="ci-desc">{state.hud.selectedDescription}</p>
            {state.hud.selectedMeta.map((entry) => (<div className="meta-row" key={entry.label}>
                <span>{entry.label}</span>
                <strong>{entry.value}</strong>
        ...
        {state.mode === 'replay' ? (<div className="ci-section">
            {state.hud.recentCombat.length > 0 ? (<>
                <p className="panel-kicker">Combat</p>
                {state.hud.recentCombat.map((event, index) => (<div className="inspector-entry" key={`${event.attackerUnitId}-${event.defenderUnitId}`}>
            ...
            {state.hud.recentIntents.length > 0 ? (<>
                ...
                {state.hud.recentIntents.map((intent) => (<div className="inspector-entry" key={intent.unitId}>
