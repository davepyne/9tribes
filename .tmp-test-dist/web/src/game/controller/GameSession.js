...
import { canPaySettlerVillageCost, canProducePrototype, cancelCurrentProduction, getPrototypeCostType, getPrototypeQueueCost, queueUnit, removeFromQueue, } from '../../../../src/systems/productionSystem.js';
...
import { startResearch, } from '../../../../src/systems/researchSystem.js';
import { unlockHybridRecipes } from '../../../../src/systems/hybridSystem.js';
import { deserializeGameState, serializeGameState } from '../types/playState.js';
...
export class GameSession {
    ...
    _aiCombatQueue = [];
    _aiTurnContext = null;
    constructor(source = { type: 'fresh' }, registry = loadRulesRegistry(), options = {}) {
    ...
    getSaveSnapshot() {
        ...
        const playerFaction = playerFactionId ? this.state.factions.get(playerFactionId) : null;
        const activeFaction = this.state.activeFactionId
            ? this.state.factions.get(this.state.activeFactionId)
            : null;
        return {
    ...
    dispatch(action) {
        switch (action.type) {
            case 'move_unit':
                this.applyMove(action.unitId, action.destination);
                return;
            case 'attack_unit':
                this._pendingCombat = this.resolveAttack(action.attackerId, action.defenderId);
                return;
            ...
            case 'end_turn':
                if (this.state.activeFactionId) {
                    ...
                    this.state = runFactionPhase(this.state, activeFactionId, this.registry, {
    ...
    getAttackTargets(unitId) {
        ...
        const prototype = this.state.prototypes.get(unit.prototypeId);
    ...
    buildReachableMoves(unitId) {
        ...
        const frontier = [{ state: this.state, path: [start] }];
        const bestRemainingByKey = new Map([[`${start.q},${start.r}`, unit.movesRemaining]]);
    ...
    applyResolvedCombat(pending) {
        ...
        this.record('combat', `${finalCombatEvent.attackerPrototypeName} attacked ${finalCombatEvent.defenderPrototypeName}: dealt ${preview.result.defenderDamage}, took ${preview.result.attackerDamage}.`);
    }
    getPendingCombat() {
    ...
    getAiCombatQueueLength() {
        return this._aiCombatQueue.length;
    }
    refreshFogForAllFactions(state) {
        let nextState = state;
    ...
    getPrototypeName(prototypeId) {
        return this.state.prototypes.get(prototypeId)?.name ?? prototypeId;
    }
    ...
    isHumanControlledFaction(factionId) {
    ...
    processAiTurnUntilBoundary(factionId) {
        if (this.state.activeFactionId !== factionId) {
            return false;
        }
        if (!this._aiTurnContext || this._aiTurnContext.factionId !== factionId) {
            const strategy = computeFactionStrategy(this.state, factionId, this.registry, this.difficulty);
            this.state = {
                ...this.state,
                factionStrategies: new Map(this.state.factionStrategies).set(factionId, strategy),
            };
            this._aiTurnContext = {
                factionId,
                unitIds: this.getAiUnitIds(factionId),
                index: 0,
                fortsBuiltThisTurn: new Set(),
            };
        }
        while (this._aiTurnContext && this._aiTurnContext.index < this._aiTurnContext.unitIds.length) {
            const unitId = this._aiTurnContext.unitIds[this._aiTurnContext.index];
            ...
            const activation = activateAiUnit(this.state, unitId, this.registry, {
                combatMode: 'preview',
                fortsBuiltThisRound: this._aiTurnContext.fortsBuiltThisTurn,
            });
        ...
        this.feedback.lastMove = null;
        this.state = runFactionPhase(this.state, factionId, this.registry, {
            difficulty: this.difficulty,
        });
        this.state = this.refreshFogForAllFactions(advanceTurn(this.state));
        this.feedback.lastActiveFactionId = this.state.activeFactionId;
    ...
    setCityProduction(cityId, prototypeId) {
        ...
        const updatedCity = queueUnit(city, prototype.id, prototype.chassisId, costType === 'villages' ? getPrototypeQueueCost(prototype) : this.getPrototypeCost(prototype.id), costType);
    ...
    removeFromQueue(cityId, queueIndex) {
        const city = this.state.cities.get(cityId);
        if (!city || city.factionId !== this.state.activeFactionId || !this.isHumanControlledFaction(city.factionId))
            return;
    ...
    applyStartResearch(nodeId) {
        const factionId = this.state.activeFactionId;
        if (!factionId || !this.isHumanControlledFaction(factionId))
            return;
        const research = this.state.research.get(factionId);
        ...
        if (!faction.learnedDomains?.includes(domainId))
            return;
        const updated = startResearch(research, nodeId, nodeDef.prerequisites, faction.learnedDomains);
        const nextResearch = new Map(this.state.research);
        nextResearch.set(factionId, updated);
    ...
    applyCancelResearch() {
        ...
        if (!factionId || !this.isHumanControlledFaction(factionId))
            return;
        const research = this.state.research.get(factionId);
        if (!research || !research.activeNodeId)
            return;
    ...
    applySacrifice(unitId) {
        const unit = this.state.units.get(unitId);
        if (!unit || !this.state.activeFactionId)
            return;
        const faction = this.state.factions.get(unit.factionId);
        if (!faction || unit.factionId !== this.state.activeFactionId)
            return;
        ...
        this.state = performSacrifice(unitId, unit.factionId, this.state, this.registry);
        ...
        this.feedback.lastMove = null;
        this.feedback.lastTurnChange = null;
        this.record('turn', `${unitName} sacrificed - unlocked domains: ${domains.join(', ')}.`);
    }
    applyBuildFort(unitId) {
        const unit = this.state.units.get(unitId);
    ...
    applyPrepareAbility(unitId, ability) {
        const unit = this.state.units.get(unitId);
        if (!unit || !this.state.activeFactionId || unit.factionId !== this.state.activeFactionId) {
            return;
        }
    ...
    applyBoardTransport(unitId, transportId) {
        const unit = this.state.units.get(unitId);
        const transport = this.state.units.get(transportId);
    ...
    applyDisembarkUnit(unitId, transportId, destination) {
        const unit = this.state.units.get(unitId);
        if (!unit || !this.state.activeFactionId || unit.factionId !== this.state.activeFactionId) {
            return;
        }
        ...
        const result = disembarkUnit(this.state, transportId, unit.id, destination, this.registry, this.state.transportMap);
        this.state = {
            ...result.state,
            transportMap: result.transportMap,
        };
    ...
    buildFortAtUnit(unit, defenseBonus) {
