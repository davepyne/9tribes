...
import { serializeGameState } from '../types/playState.js';
const PLAYTEST_SEED = 42;
const PLAYTEST_FACTIONS = ['druid_circle', 'steppe_clan'];
export function createCuratedPlaytestPayload() {
...
function curatePlaytestState(state) {
    const factionIds = new Set(PLAYTEST_FACTIONS);
    ...
    const druidFaction = state.factions.get('druid_circle');
    const steppeFaction = state.factions.get('steppe_clan');
    ...
    const druidCity = state.cities.get(druidFaction.cityIds[0]);
    const steppeCity = state.cities.get(steppeFaction.cityIds[0]);
...
function patchTerrain(state, position, terrain) {
