import { parseFreecivTagFrameLookup } from './freelandSpec.js';
...
export function getFogRenderState(visibility) {
    return VISIBILITY_TO_FOG_STATE[visibility];
}
export function getFogTag(north, east, south, west) {
    return `t.fog_${north}_${east}_${south}_${west}`;
}
const frame = (row, col, columns) => row * columns + col;
...
function getRequiredSpecText(scene, key) {
...
export function initializeFreelandTerrainFrames(scene) {
    const nextFrames = { ...TERRAIN_FRAMES };
    for (const [terrainKey, specKey] of Object.entries(TERRAIN_FRAME_SPEC_KEYS)) {
...
const CITY_COLUMN = 3;
const SETTLEMENT_ROW_BY_FACTION = {
    steppe_clan: 0,
    jungle_clan: 1,
    plains_riders: 1,
    savannah_lions: 1,
...
export function getSettlementFrame(factionId, kind) {
...
export function getUnitTextureSpec(spriteKey) {
    const textureConst = PLAYTEST_SPRITE_KEYS[spriteKey];
    if (textureConst) {
        return {
            kind: 'image',
            texture: TEXTURES[textureConst],
            displayWidth: 48,
            displayHeight: 64,
            yOffset: 8,
        };
...
export function getUnitRearTextureSpec(spriteKey) {
...
function encodeRiverConnectionFlags(flags) {
    return (flags.north ? 8 : 0)
        + (flags.east ? 4 : 0)
        + (flags.south ? 2 : 0)
        + (flags.west ? 1 : 0);
}
export function getRiverOverlayFrameForTile(q, r, getTerrainAt) {
    const isRiverConnection = (dq, dr) => {
...
export function getTerrainRenderSpec(terrain) {
    switch (terrain) {
        case 'forest':
            return {
