...
import { TILE_HALF_HEIGHT, TILE_HALF_WIDTH } from '../assets/keys.js';
import { BorderRenderer } from '../systems/BorderRenderer.js';
import { CombatAnimator } from '../systems/CombatAnimator.js';
import { FogRenderer } from '../systems/FogRenderer.js';
import { ImprovementRenderer } from '../systems/ImprovementRenderer.js';
import { PathRenderer } from '../systems/PathRenderer.js';
import { SelectionRenderer } from '../systems/SelectionRenderer.js';
import { SettlementRenderer } from '../systems/SettlementRenderer.js';
import { TileLayerRenderer } from '../systems/TileLayerRenderer.js';
import { UnitRenderer } from '../systems/UnitRenderer.js';
export class MapScene extends Phaser.Scene {
    ...
    fogRenderer;
    dragOrigin = null;
    rightButtonDownThisFrame = false;
    ...
    renderFromState(state) {
        ...
        this.selectionRenderer.render(state.world, state.selected, state.hoveredHex ? `${state.hoveredHex.q},${state.hoveredHex.r}` : null);
        this.fogRenderer.render(state.world);
        this.layoutCamera(state);
    }
    handleHexClick(state, q, r, pointer) {
        if (this.combatAnimator.isAnimating())
            return;
        // Right-click is handled by handleRightClick — ignore here to avoid eating the selection
        if (MapScene.isRightClick(pointer))
            return;
        const key = `${q},${r}`;
    ...
    findPlayerStart(state) {
        ...
        if (activeFaction?.homeCityId) {
            ...
            if (homeCity)
                return { q: homeCity.q, r: homeCity.r };
        }
        // Fall back to any city owned by the active faction
        const factionCity = state.world.cities.find((c) => c.factionId === state.world.activeFactionId && c.visible);
        if (factionCity)
            return { q: factionCity.q, r: factionCity.r };
        // Fall back to any visible unit belonging to the active faction
        const activeUnit = state.world.units.find((u) => u.isActiveFaction && u.visible);
        if (activeUnit)
            return { q: activeUnit.q, r: activeUnit.r };
    ...
    handleUnitSelection(state, unitId, pointer) {
        if (this.combatAnimator.isAnimating())
            return;
        // Right-click is handled by handleRightClick — ignore here to avoid eating the selection
        if (MapScene.isRightClick(pointer))
    ...
    handleCitySelection(state, cityId, pointer) {
        if (this.combatAnimator.isAnimating())
            return;
        if (MapScene.isRightClick(pointer))
            return;
    ...
    handleSingleClickUnit(state, unitId) {
        const unit = state.world.units.find((entry) => entry.id === unitId);
        if (!unit) {
            return;
        }
    ...
    handleUnitPointerDown(_state, _unitId, _pointer) {
    ...
    startCombatAnimation(data, attackerView, defenderView, onComplete, skipAnimation = false) {
