...
export class SelectionRenderer {
    ...
    render(world, selected, hoveredKey) {
        ...
        if (hoveredKey && !reachableKeys.has(hoveredKey) && !attackKeys.has(hoveredKey)) {
            ...
            this.layer.add(this.scene.add.image(point.x, point.y, TEXTURES.selection, 1)
                .setOrigin(0.5, 1)
                .setAlpha(0.16)
                .setTint(0xd8c06f));
        }
        ...
        this.layer.add(this.scene.add.image(point.x, point.y, TEXTURES.selection, 0)
            .setOrigin(0.5, 1)
            .setAlpha(0.92)
            .setTint(selected.type === 'unit' ? 0xf7e7bf : 0xd9b86a));
    }
}
function resolveSelectionPosition(world, selected) {
    if (!selected) {
        return null;
    }
