import { buildCombatAnimationScript, } from './combatAnimationScript.js';
import { getUnitTextureSpec, getUnitRearTextureSpec } from '../assets/keys.js';
export class CombatAnimator {
    scene;
    worldToScreen;
    overlayLayer;
    active = null;
    constructor(scene, worldToScreen) {
        this.scene = scene;
    ...
    playCombat(data, attackerView, defenderView, onComplete, skipAnimation = false) {
        if (skipAnimation) {
        ...
        allSprites.push(attSprite, defSprite, attMarker, defMarker, attHpBar.container, defHpBar.container);
        ...
        const tweenTargetsTo = (targets, config) => {
        ...
        script.beats.forEach((beat, index) => {
            ...
            tweenTargetsTo([
                { obj: actorSprite, point: actorPeak },
                { obj: actorMarker, point: actorMarkerPeak },
                { obj: actorHpBar.container, point: actorHpBarPeak },
            ], {
            ...
            tweenTargetsTo([
                { obj: targetSprite, point: targetPeak },
                { obj: targetMarker, point: targetMarkerPeak },
                { obj: targetHpBar.container, point: targetHpBarPeak },
            ], {
        ...
        if (defenderDead && !attackerDead) {
            ...
            tweenTargetsTo([
                { obj: attSprite, point: defSpriteStart },
                { obj: attMarker, point: defMarkerStart },
                { obj: attHpBar.container, point: defHpBarStart },
            ], {
                duration: 420,
                ease: 'Quad.easeOut',
                delay: 1540,
            ...
            tweenTargetsTo([
                { obj: attSprite, point: pursuit },
                { obj: attMarker, point: pursuitMarker },
                { obj: attHpBar.container, point: pursuitHpBar },
            ], {
                duration: 360,
                ease: 'Quad.easeOut',
                delay: 1540,
            ...
            tweenTargetsTo([
                { obj: attSprite, point: attSpriteStart },
                { obj: attMarker, point: attMarkerStart },
                { obj: attHpBar.container, point: attHpBarStart },
            ], {
                duration: 380,
                ease: 'Quad.easeIn',
                delay: 1500,
    ...
    playInstant(data, attackerView, defenderView, onComplete) {
        this.cancel();
        const attPos = this.worldToScreen(attackerView.q, attackerView.r);
        const defPos = this.worldToScreen(defenderView.q, defenderView.r);
        this.overlayLayer.removeAll(true);
        ...
        script.beats
            ...
            const damageText = this.createDamageText({ x: targetPos.x + index * 12 - 12, y: targetPos.y - 44 - index * 10 }, beat.damage).setAlpha(1).setVisible(true);
            allSprites.push(damageText);
        });
        for (const obj of allSprites) {
            this.overlayLayer.add(obj);
    ...
    createImpactFlash(pos, intensity, isGlance) {
        return this.scene.add.ellipse(pos.x, pos.y, 16 + intensity * 14, 10 + intensity * 8, isGlance ? 0xc9d6e1 : 0xf5d784, 0);
    }
    normalize(vector) {
