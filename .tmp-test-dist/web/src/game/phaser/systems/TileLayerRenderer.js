import { getRiverOverlayFrameForTile, getTerrainRenderSpec, TEXTURES, TILE_HEIGHT, TILE_WIDTH } from '../assets/keys.js';
    ...
    render(world, state, callbacks) {
        ...
        for (const hex of sortedHexes) {
            ...
            const spec = hex.terrain === 'river'
                ? {
                    ...baseSpec,
                    overlayFrame: getRiverOverlayFrameForTile(hex.q, hex.r, (q, r) => terrainByKey.get(`${q},${r}`)),
                }
                : baseSpec;
            const hit = this.scene.add.polygon(point.x, point.y, [0, -TILE_HEIGHT / 2, TILE_WIDTH / 2, 0, 0, TILE_HEIGHT / 2, -TILE_WIDTH / 2, 0], spec.fallbackColor, 0.24)
                .setStrokeStyle(1, 0xf7e7bf, 0.12)
                .setInteractive({ cursor: reachableKeys.has(hex.key) ? 'pointer' : 'help' });
            hit.on('pointerdown', (pointer) => callbacks.onHexSelected(hex.q, hex.r, pointer));
