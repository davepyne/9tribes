...
import { deleteSaveGame, listSaveGames } from '../savegames.js';
import { getMvpFactionConfigs } from '../../../../src/game/scenarios/mvp.js';
import { playMenuUiSound } from '../audio/musicManager.js';
const FACTIONS = getMvpFactionConfigs();
const MENU_STEPS = ['title', 'mode', 'setup', 'tribe'];
...
export function MenuClient() {
    ...
    const selectedFaction = useMemo(() => FACTIONS.find((faction) => faction.id === setup.selectedFactionId) ?? FACTIONS[0], [setup.selectedFactionId]);
    ...
    return (<main className={`menu-shell menu-shell--${step}`}>
      <div className={`menu-screen ${step === 'title' ? 'menu-screen--title' : 'menu-screen--panel'}`}>
        {step !== 'title' ? (<header className="menu-header">
        ...
        {step === 'title' ? (<section className="menu-title">
            <div className="menu-title__scrim"/>
            <div className="menu-title__actions">
              <button className="menu-primary menu-primary--hero" type="button" onClick={() => {
                playUiSelect();
                setStep('mode');
            }}>
                Press Start
        ...
        {step === 'mode' ? (<section className="menu-panel">
            ...
            <div className="menu-option-stack">
              <button className={`menu-option ${mode === 'singleplayer' ? 'is-selected' : ''}`} type="button" onClick={() => {
                playUiSelect();
                setStep('setup');
            }}>
              ...
              <button className="menu-option" type="button" onClick={() => {
                playUiSelect();
                setStep('load');
            }}>
        ...
        {step === 'setup' ? (<section className="menu-panel">
            <div className="menu-copy">
            ...
            <div className="menu-config-grid">
              <div className="menu-config-card">
                ...
                <div className="menu-chip-row">
                  {['easy', 'normal', 'hard'].map((difficulty) => (<button key={difficulty} className={`menu-chip ${setup.difficulty === difficulty ? 'is-selected' : ''}`} type="button" onClick={() => {
                    playUiSelect();
              ...
              <div className="menu-config-card">
                <h3>Map Size</h3>
                <div className="menu-chip-row">
                  {['small', 'medium', 'large'].map((mapSize) => (<button key={mapSize} className={`menu-chip ${setup.mapSize === mapSize ? 'is-selected' : ''}`} type="button" onClick={() => {
                    playUiSelect();
            ...
            <div className="menu-footer-actions">
              <button className="menu-primary" type="button" onClick={() => {
                playUiSelect();
                setStep('tribe');
            }}>
                Choose Tribe
        ...
        {step === 'tribe' ? (<section className="menu-panel menu-panel--tribes">
            <div className="menu-copy">
            ...
            <div className="tribe-layout">
              <div className="tribe-grid" role="list" aria-label="Playable tribes">
                ...
                return (<button key={faction.id} className={`tribe-card ${isSelected ? 'is-selected' : ''}`} type="button" onClick={() => {
                        playUiSelect();
                        setSetup((current) => ({ ...current, selectedFactionId: faction.id }));
                    }} role="listitem">
                      <span className="tribe-card__crest" style={{ '--tribe-accent': faction.color }}/>
              ...
              {selectedFaction ? (<aside className="tribe-preview">
                  <div className="tribe-preview__swatch" style={{ '--tribe-accent': selectedFaction.color }}/>
        ...
        {step === 'load' ? (<section className="menu-panel">
            <div className="menu-copy">
            ...
            {saveGames.length > 0 ? (<div className="savegame-list">
                {saveGames.map((saveGame) => (<article key={saveGame.id} className="savegame-card">
              ...
              </div>) : (<div className="menu-empty-state">
