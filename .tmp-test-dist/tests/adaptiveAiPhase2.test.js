...
import { getProjectedSupplyMarginAfterBuild, getSupplyMargin, rankProductionPriorities, } from '../src/systems/aiProductionStrategy.js';
import { rankResearchPriorities } from '../src/systems/aiResearchStrategy.js';
import { getFactionProjectedSupplyDemand } from '../src/systems/productionSystem.js';
const registry = loadRulesRegistry();
function getPrototypeByChassis(state, factionId, chassisId) {
    return Array.from(state.prototypes.values()).find((prototype) => prototype.factionId === factionId && prototype.chassisId === chassisId);
}
describe('adaptive AI phase 2', () => {
    it('keeps settler production off on easy and makes it more attractive for defensive tribes on normal', () => {
        ...
        state.factions.set(hillId, {
            ...hillFaction,
            villageIds: ['hill_v1', 'hill_v2', 'hill_v3', 'hill_v4'],
        });
        state.factions.set(steppeId, {
            ...steppeFaction,
            villageIds: ['steppe_v1', 'steppe_v2', 'steppe_v3', 'steppe_v4'],
        });
        state.round = 12;
        const hillSettler = Array.from(state.prototypes.values()).find((prototype) => prototype.factionId === hillId && prototype.tags?.includes('settler'));
        const steppeSettler = Array.from(state.prototypes.values()).find((prototype) => prototype.factionId === steppeId && prototype.tags?.includes('settler'));
    ...
    it('shifts mounted-heavy research toward logistics-friendly domains when supply is tight', () => {
        ...
        state.research.set(factionId, {
            ...research,
            completedNodes: [...new Set([...research.completedNodes, 'charge_t1'])],
        });
        const projectedDemand = getFactionProjectedSupplyDemand(state, factionId, registry);
        state.economy.set(factionId, {
            factionId,
    ...
    it('pushes normal research toward foreign breadth before native tier 3 when breadth is undeveloped', () => {
        ...
        state.research.set(factionId, {
            ...research,
            completedNodes: [...new Set([...research.completedNodes, 'hitrun_t2', 'charge_t1'])],
        });
        state.round = 40;
    ...
    it('keeps hard settler windows tighter than normal when supply and reserves are weak', () => {
        const state = buildMvpScenario(42, { registry });
        const factionId = 'hill_clan';
        const faction = state.factions.get(factionId);
        const settler = Array.from(state.prototypes.values()).find((prototype) => prototype.factionId === factionId && prototype.tags?.includes('settler'));
