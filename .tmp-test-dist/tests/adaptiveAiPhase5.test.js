...
import { initializeFogForFaction, updateFogState } from '../src/systems/fogSystem.js';
...
function trimState(state, factionIds) {
    ...
    for (const factionId of keepFactions) {
        current = initializeFogForFaction(current, factionId);
    }
    return current;
}
function withUpdatedFog(state, factionIds) {
    let current = state;
...
describe('adaptive AI phase 5', () => {
    it('holds/regroups when squad mass is insufficient to commit', () => {
        ...
        const waitedUnits = Object.values(strategy.unitIntents).filter((intent) => intent.reason.includes('wait_for_allies=holding_for_squad'));
    ...
    it('allows doctrine override for exceptional hitrun/charge opportunities', () => {
        ...
        const withFog = withUpdatedFog(trimmed, [steppeId, hillId]);
        const strategy = computeFactionStrategy(withFog, steppeId, registry);
        const overrideIntents = Object.values(strategy.unitIntents).filter((intent) => intent.reason.includes('doctrine_override=exceptional_opportunity'));
    ...
    it('emits stable phase-5 telemetry markers and remains deterministic across runs', () => {
        ...
        const normalize = (events) => events?.map((event) => ({
            ...
            reasons: event.reasons
                .filter((reason) => reason.startsWith('assignment_mix=')
