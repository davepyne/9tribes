import civilizationsData from '../content/base/civilizations.json' with { type: 'json' };
import { getVictoryStatus } from '../systems/warEcologySimulation.js';
const CIVILIZATIONS = civilizationsData;
export function exportReplayBundle(finalState, trace, maxTurns) {
    const map = finalState.map;
    if (!map) {
        throw new Error('Cannot export replay bundle without a map.');
    }
    const factions = Array.from(finalState.factions.values()).map((faction) => {
        const civConfig = CIVILIZATIONS[faction.id];
        return {
            id: faction.id,
            name: faction.name,
    ...
    const turns = Array.from(snapshotsByRound.entries())
...
function toReplaySnapshot(snapshot, finalState) {
    return {
