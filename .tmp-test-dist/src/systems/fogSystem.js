 ...
 */
export function calculateVisibility(state, factionId) {
    ...
    const hexVisibility = new Map();
    const lastSeen = new Map(previousLastSeen);
 ...
 */
function captureLastSeenSnapshot(state, key, lastSeen) {
 ...
 */
export function getLastSeenEnemyUnits(state, factionId) {
    ...
    for (const [key, snapshot] of fogState.lastSeen) {
        if (!snapshot.unit)
            continue;
        if (snapshot.unit.factionId === factionId)
            continue;
 ...
 */
export function getLastSeenEnemyCities(state, factionId) {
    const fogState = state.fogState?.get(factionId);
    if (!fogState)
        return [];
    const result = [];
    ...
    for (const [key, snapshot] of fogState.lastSeen) {
        if (!snapshot.city)
            continue;
        if (snapshot.city.factionId === factionId)
            continue;
        const position = keyToHex(key);
        // Find the actual city to get its id
        const actualCity = Array.from(state.cities.values()).find((c) => c.position.q === position.q && c.position.r === position.r);
        result.push({
            position,
 ...
 */
export function getExploredHexKeys(state, factionId) {
    const fogState = state.fogState?.get(factionId);
    if (!fogState)
        return new Set();
    const keys = new Set();
 ...
 */
export function getHexVisibility(state, factionId, hex) {
    const fogState = state.fogState?.get(factionId);
 ...
 */
export function isUnitVisibleTo(state, factionId, unit) {
 ...
 */
export function getVisibleHexKeys(state, factionId) {
    const fogState = state.fogState?.get(factionId);
    if (!fogState)
        return new Set();
 ...
 */
export function initializeFogForFaction(state, factionId) {
 ...
 */
export function updateFogState(state, factionId) {
