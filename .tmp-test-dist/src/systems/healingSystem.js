...
function hasFriendlyNatureHealingAura(state, factionId, unit, radius) {
 ...
 */
export function applyHealingForFaction(gameState, factionId, _registry) {
    ...
    for (const unitIdStr of faction.unitIds) {
        ...
        if (tags.includes('druid') || tags.includes('healing')) {
            ...
            const auraSource = hasFriendlyNatureHealingAura(gameState, factionId, unit, doctrine.healingAuraUpgradeEnabled ? 2 : 1);
            if (auraSource) {
        ...
        unitsMap.set(unitIdStr, refreshedUnit);
        changed = true;
    }
