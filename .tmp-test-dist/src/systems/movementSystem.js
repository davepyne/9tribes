...
let movementSynergyEngine = null;
function getMovementSynergyEngine() {
    if (!movementSynergyEngine) {
        movementSynergyEngine = new SynergyEngine(pairSynergiesData.pairSynergies, emergentRulesData.rules, Object.values(abilityDomainsData.domains));
    }
    return movementSynergyEngine;
}
function getSwarmSpeedBonus(tags) {
    const engine = getMovementSynergyEngine();
    const synergies = engine.resolveUnitPairs(tags);
    for (const syn of synergies) {
        if (syn.effect.type === 'swarm_speed') {
            return syn.effect.speedBonus;
        }
    }
    return 0;
}
export function previewMove(gameState, unitId, targetHex, map, rulesRegistry) {
    ...
    const doctrine = resolveResearchDoctrine(gameState.research.get(unit.factionId), faction);
 ...
 */
export function canMoveTo(gameState, unitId, targetHex, map, rulesRegistry) {
    // Get the unit
    const unit = gameState.units.get(unitId);
    if (!unit || unit.status !== 'ready') {
        return false;
 ...
 */
export function moveUnit(gameState, unitId, targetHex, map, rulesRegistry) {
    if (!canMoveTo(gameState, unitId, targetHex, map, rulesRegistry)) {
        throw new Error(`Invalid move for unit ${unitId} to (${targetHex.q}, ${targetHex.r})`);
    }
    const unit = gameState.units.get(unitId);
    const preview = previewMove(gameState, unitId, targetHex, map, rulesRegistry);
 ...
 */
export function getValidMoves(gameState, unitId, map, rulesRegistry) {
    const unit = gameState.units.get(unitId);
    if (!unit || unit.status !== 'ready') {
        return [];
    }
    const neighbors = getNeighbors(unit.position);
    return neighbors.filter((hex) => canMoveTo(gameState, unitId, hex, map, rulesRegistry));
}
