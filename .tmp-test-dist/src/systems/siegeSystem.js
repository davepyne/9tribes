 ...
 */
export function degradeWalls(city, isCoastalWalls = false) {
    if (!city.besieged)
        return city;
 ...
 */
export function repairWalls(city) {
    if (city.besieged)
        return city;
    if (city.wallHP >= city.maxWallHP)
        return city;
    return {
        ...city,
 ...
 */
export function getWallDefenseBonus(state, position, wallDefenseMultiplier = 2) {
 ...
 */
export function hasDefendingGarrison(city, state) {
    for (const [, unit] of state.units) {
        if (unit.factionId !== city.factionId || unit.hp <= 0)
            continue;
 ...
 */
export function isCityVulnerable(city, state) {
 ...
 */
export function getCapturingFaction(city, state) {
    ...
    for (const hex of neighbors) {
        for (const [, unit] of state.units) {
            if (unit.hp > 0 &&
                ...
                adjacentCounts.set(unit.factionId, (adjacentCounts.get(unit.factionId) ?? 0) + 1);
            }
        }
    }
    if (adjacentCounts.size === 0)
        return null;
    let maxFaction = null;
    let maxCount = 0;
 ...
 */
export function captureCity(city, newOwnerFactionId, state) {
    ...
    if (victimWE) {
        warExhaustion.set(oldOwnerFactionId, addExhaustion(victimWE, EXHAUSTION_CONFIG.CITY_CAPTURED));
    }
    const attackerWE = warExhaustion.get(newOwnerFactionId);
    if (attackerWE) {
        warExhaustion.set(newOwnerFactionId, addExhaustion(attackerWE, EXHAUSTION_CONFIG.CITY_CAPTURED_ATTACKER));
    }
    let nextState = {
        ...state,
        cities,
