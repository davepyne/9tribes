...
import { canUseAmbush, canUseBrace, canUseCharge, clearPreparedAbility, getTerrainAt as getAbilityTerrainAt, hasAdjacentEnemy, prepareAbility, shouldClearAmbush, } from './abilitySystem.js';
...
import { canBoardTransport, boardTransport, disembarkUnit, getEmbarkedUnits, getValidDisembarkHexes, isTransportUnit, isUnitEmbarked, updateEmbarkedPositions, } from './transportSystem.js';
...
import { computeRetreatRisk, scoreAttackCandidate, scoreMoveCandidate, scoreStrategicTarget, shouldEngageTarget, } from './aiTactics.js';
import { log, recordAiIntent, recordCombatEvent, } from './warEcologySimulation.js';
...
function describeCombatOutcome(result) {
    ...
    if (result.defenderFled)
        return 'Defender fled';
    if (result.defenderRouted)
        return 'Defender routed';
    if (result.attackerFled)
        return 'Attacker fled';
    if (result.attackerRouted)
        return 'Attacker routed';
    return 'Exchange';
}
function formatCombatSummary(attackerName, defenderName, defenderDamage, attackerDamage, outcome, effects) {
...
function rotateUnitToward(unit, target) {
...
function shouldBrace(unit, prototype, state, canUniversalBrace = false) {
    if ((!canUseBrace(prototype) && !canUniversalBrace) || hasAdjacentEnemy(state, unit) === false) {
        return false;
    }
...
export function maybeExpirePreparedAbility(unit, round, state) {
 ...
 */
function getImprovementBonus(state, pos) {
...
function getImprovementAtHex(state, pos) {
    for (const [, improvement] of state.improvements) {
        if (improvement.position.q === pos.q && improvement.position.r === pos.r) {
            return improvement;
        }
...
function isFortificationHex(state, pos) {
    return getImprovementAtHex(state, pos)?.type === 'fortification';
}
function countFriendlyUnitsNearHex(state, factionId, pos, radius, excludedUnitId) {
    let count = 0;
    for (const unit of state.units.values()) {
        if (unit.hp <= 0 || unit.factionId !== factionId)
            continue;
        if (excludedUnitId && unit.id === excludedUnitId)
            continue;
...
function countFortificationsNearHex(state, pos, radius) {
    let count = 0;
...
function countUnitsNearHex(state, pos, radius, predicate) {
    let count = 0;
    for (const unit of state.units.values()) {
        if (unit.hp <= 0)
            continue;
...
function findBestTargetChoice(state, unitId, position, friendlyFactionId, myPrototype, registry) {
    let bestTarget = null;
    ...
    for (const targetPos of getNeighbors(position)) {
        for (const [, unit] of state.units) {
            if (unit.factionId !== friendlyFactionId &&
                unit.hp > 0 &&
                ...
                if (isUnitRiverStealthed(targetFaction, targetTerrain))
                    continue;
                ...
                const myWeaponTags = [];
                for (const compId of myPrototype.componentIds ?? []) {
                    const comp = registry.getComponent(compId);
                    if (comp?.slotType === 'weapon' && comp.tags)
                        myWeaponTags.push(...comp.tags);
                }
                ...
                const strategicScore = scoreStrategicTarget({
                    ...
                    isAdjacentToPrimaryObjectiveCity: Boolean(strategy?.primaryCityObjectiveId
                        && Array.from(state.cities.values()).some((city) => city.id === strategy.primaryCityObjectiveId
                    ...
                    attacksFromThreatenedCityHex: Boolean(strategy?.threatenedCities.some((threat) => {
...
function findBestRangedTarget(state, unitId, position, friendlyFactionId, myPrototype, registry, range) {
    let bestTarget = null;
    let bestScore = -Infinity;
    const strategy = state.factionStrategies.get(friendlyFactionId);
    const actingUnit = state.units.get(unitId);
    ...
    for (const hex of hexesInRange) {
        ...
        if (hex.q === position.q && hex.r === position.r)
            continue;
        for (const [, unit] of state.units) {
            if (unit.factionId !== friendlyFactionId &&
                unit.hp > 0 &&
                ...
                const isOnCity = [...state.cities.values()].some((city) => city.position.q === unit.position.q && city.position.r === unit.position.r);
...
function getNearestFriendlyDistanceToHex(state, factionId, hex, excludedUnitId) {
...
function countNearbyUnitPressure(state, factionId, hex, excludedUnitId) {
    let nearbyEnemies = 0;
...
function setUnitActivated(state, unitId) {
...
function buildFieldFortIfEligible(state, factionId, unitId, registry, fortsBuiltThisRound) {
    const faction = state.factions.get(factionId);
    const unit = state.units.get(unitId);
    ...
    const isDefensiveAssignment = unitIntent?.assignment === 'defender'
        || unitIntent?.assignment === 'recovery'
        || unitIntent?.assignment === 'reserve';
    const worthwhile = nearbyFriendlySupport > 0
        && nearbyFortCount === 0
        && (nearbyEnemies > 0
...
function applyHillDugInIfEligible(state, factionId, unitId) {
    const faction = state.factions.get(factionId);
    const research = state.research.get(factionId);
    const unit = state.units.get(unitId);
...
function performStrategicMovement(state, unitId, registry, trace) {
    const unit = state.units.get(unitId);
 ...
 */
function moveTransportAndDisembark(state, transportId, registry, waypoint, unitIntent, trace) {
    if (!state.map)
    ...
    let bestMove = null;
    let bestScore = -Infinity;
    const originWaypointDistance = hexDistance((state.units.get(transportId) ?? { position: waypoint }).position, waypoint);
    for (const move of validMoves) {
        const waypointDistance = hexDistance(move, waypoint);
        ...
        if (terrainId === 'coast' || terrainId === 'river')
            score += 1;
        if (score > bestScore) {
            bestScore = score;
            bestMove = move;
 ...
 */
function autoDisembark(state, transportId, registry, trace) {
...
function buildFallbackIntent(state, unit) {
...
function resolveWaypoint(state, unit, intent) {
...
function wouldBeUnsafeAfterMove(state, unit, move, intent) {
...
function mapAssignmentToIntent(intent) {
    if (intent.assignment === 'recovery')
        return 'retreat';
    if (intent.assignment === 'defender' || intent.assignment === 'reserve')
        return 'regroup';
    if (intent.assignment === 'siege_force')
        return 'siege';
    if (intent.assignment === 'raider')
        return 'support';
    return 'advance';
}
export function activateUnit(state, unitId, registry, options = {}) {
    ...
    const canChargeAttack = unitRange <= 1 && (canUseCharge(prototype) || factionDoctrine.chargeTranscendenceEnabled);
    const shouldEngageFromPosition = (unitAtPosition, attackScore) => {
    ...
    let enemyChoice = unitRange > 1
        ? findBestRangedTarget(current, unitId, activeUnit.position, factionId, prototype, registry, unitRange)
        : findBestTargetChoice(current, unitId, activeUnit.position, factionId, prototype, registry);
    let enemy = enemyChoice.target;
    ...
    if (!enemy && activeUnit.movesRemaining > 0 && canChargeAttack) {
        ...
        for (const move of getValidMoves(current, unitId, map, registry)) {
            const choice = findBestTargetChoice(current, unitId, move, factionId, prototype, registry);
            if (!choice.target)
                continue;
        ...
        if (chargeMove && bestChargeScore > 0) {
            ...
            enemyChoice = findBestTargetChoice(current, unitId, activeUnit.position, factionId, prototype, registry);
    ...
    if (enemy &&
        ...
        shouldBrace(activeUnit, prototype, current, factionDoctrine.fortressTranscendenceEnabled) &&
        enemyChoice.score <= 0) {
        const units = new Map(current.units);
    ...
    if (enemy && activeUnit.attacksRemaining > 0) {
        ...
        log(trace, `${faction.name} ${prototype.name} fought ${enemyPrototype.name}${roleInfo}${weaponInfo}${rearInfo}${abilityInfo}${stealthInfo}${poisonInfo}${knockbackInfo}${strikeFirstInfo}${moraleInfo} | capabilities: ${describeCapabilityLevels(current.factions.get(factionId))}`);
        const outcomeLabel = describeCombatOutcome(result);
        const summary = formatCombatSummary(prototype.name, enemyPrototype.name, result.defenderDamage, result.attackerDamage, outcomeLabel, resolution.triggeredEffects);
    ...
    if (isTransportUnit(prototype, registry)
        ...
        for (const [, enemy] of current.units) {
            ...
            if (dist < bestCaptureDist || (dist === bestCaptureDist && enemy.hp < (currentBest?.hp ?? Infinity))) {
                ...
                bestCaptureTarget = enemy.id;
            }
        }
        if (bestCaptureTarget) {
            const captureResult = attemptNonCombatCapture(current, unitId, bestCaptureTarget, registry, nonCombatChance, hpFraction, captureCooldown, current.rngState);
...
export function activateAiUnit(state, unitId, registry, options = {}) {
