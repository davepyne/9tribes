...
import { canPaySettlerVillageCost, getAvailableProductionPrototypes, getPrototypeCostType, getPrototypeQueueCost, getPrototypeEconomicProfile, getProjectedSupplyDemandWithPrototype, getUnitCost, isSettlerPrototype, SETTLER_VILLAGE_COST, } from './productionSystem.js';
...
function isMilitaryPrototype(prototype) {
...
function getSupplyUtilizationRatio(economy) {
...
function getTargetArmySize(state, factionId) {
...
function getProductionCostForPrototype(prototype, faction) {
...
export function getSupplyMargin(economy) {
    return Number((economy.supplyIncome - economy.supplyDemand).toFixed(2));
}
export function getProjectedSupplyMarginAfterBuild(state, factionId, prototype, registry) {
...
export function scoreSupplyEfficiency(prototype, registry) {
    const economic = getPrototypeEconomicProfile(prototype, registry);
    const combatValue = prototype.derivedStats.attack * 1.2 +
...
export function scoreForceProjectionValue(prototype, strategy) {
    const tags = prototype.tags ?? [];
...
export function rankProductionPriorities(state, factionId, strategy, registry, difficulty) {
    ...
    const recentCodifiedDomains = difficultyProfile.adaptiveAi
    ...
    return availablePrototypes
        ...
        const emergentRuleCompletionScore = scoreEmergentRuleCompletionFit(domains, faction.activeTripleStack?.domains, difficultyProfile);
        const codifiedPivotScore = scoreRecentCodifiedDomainPivot(domains, recentCodifiedDomains, difficultyProfile.production.codifiedPivotScoringBonus);
        ...
        const supplyEfficiencyScore = scoreSupplyEfficiency(prototype, registry) * difficultyProfile.production.supplyEfficiencyWeight;
        const forceProjectionScore = scoreForceProjectionValue(prototype, strategy) * difficultyProfile.production.forceProjectionWeight;
        const underCapScore = scoreUnderCapPressure(prototype, totalCost, economic.supplyCost, scoringContext, difficultyProfile);
        const armySizePressure = scoreArmySizePressure(prototype, totalFriendlyUnits, targetArmySize, difficultyProfile);
        const qualityLagScore = scoreArmyQualityLag(prototype, totalCost, scoringContext, difficultyProfile);
        const doctrineScore = scoreProductionCandidate(strategy.personality, { supplyDeficit: currentSupplyDeficit }, {
            role,
            tags: prototype.tags ?? [],
        ...
        const reason = buildProductionReason(strategy.posture, role, totalCost, hybridScore, enemyCounterPressure, roleNeed, projectedSupplyMargin, codifiedPivotScore, settlerScore, underCapScore, armySizePressure, qualityLagScore);
        return {
...
export function chooseStrategicProduction(state, factionId, strategy, registry, difficulty) {
    ...
    if (!best)
        return null;
    const prototype = state.prototypes.get(best.prototypeId);
    const faction = state.factions.get(factionId);
    if (!prototype || !faction)
        return null;
    const cost = calculatePrototypeCost(getUnitCost(prototype.chassisId), faction, getDomainIdsByTags(prototype.tags ?? []));
    return {
        prototypeId: prototype.id,
        chassisId: prototype.chassisId,
...
function scoreUnderCapPressure(prototype, totalCost, supplyCost, context, difficultyProfile) {
    if (!difficultyProfile.adaptiveAi)
        return 0;
    if (!isMilitaryPrototype(prototype))
        return 0;
    if (context.supplyUtilizationRatio >= difficultyProfile.production.underCapUtilizationFloor)
        return 0;
    const armyShortfall = Math.max(0, context.targetArmySize - context.totalFriendlyUnits);
    const unusedSupplyPressure = Math.max(0, difficultyProfile.production.underCapTargetUtilization - context.supplyUtilizationRatio);
    const cheapSupplyBonus = Math.max(0, 3 - supplyCost) * difficultyProfile.production.underCapCheapSupplyWeight;
    const cheapProductionBonus = Math.max(0, 42 - totalCost) * difficultyProfile.production.underCapCheapProductionWeight;
    return (unusedSupplyPressure * difficultyProfile.production.underCapPressureWeight
        + Math.min(difficultyProfile.production.underCapArmyShortfallCap, armyShortfall * difficultyProfile.production.underCapArmyShortfallWeight)
        + cheapSupplyBonus
        + cheapProductionBonus);
}
function scoreArmySizePressure(prototype, totalFriendlyUnits, targetArmySize, difficultyProfile) {
    if (!isMilitaryPrototype(prototype))
        return 0;
...
function scoreArmyQualityLag(prototype, totalCost, context, difficultyProfile) {
    if (!difficultyProfile.adaptiveAi)
        return 0;
    if (!isMilitaryPrototype(prototype))
        return 0;
    ...
    if (highestLag <= 0 && averageLag <= 0)
        return 0;
    let score = 0;
    if (context.highestAvailableMilitaryCost > 0
...
function rankRushProductionPriorities(state, factionId, strategy, registry, prototypes) {
...
function isRushMilitaryPrototype(prototype) {
    return isMilitaryPrototype(prototype);
}
function scoreSettlerExpansionValue(state, factionId, strategy, prototype, difficultyProfile, difficulty) {
    if (!isSettlerPrototype(prototype)) {
        return 0;
    }
    if (difficulty === 'easy') {
    ...
    const totalFriendlyUnits = Array.from(state.units.values()).filter((unit) => unit.factionId === factionId && unit.hp > 0).length;
    const targetArmySize = getTargetArmySize(state, factionId);
    const cityCount = state.factions.get(factionId)?.cityIds.length ?? 0;
    const reserveThreshold = Math.max(difficultyProfile.production.settlerReserveFloor, cityCount * difficultyProfile.production.settlerReservePerCity);
    const gateStrength = difficultyProfile.production.settlerGateStrength;
    const postureBonus = strategy.posture === 'defensive' ? 10
        : strategy.posture === 'recovery' ? 8
            : strategy.posture === 'balanced' ? 4
                : strategy.posture === 'exploration' ? 2
                    : strategy.posture === 'offensive' ? -8
                        : -4;
    const armyShortfallPenalty = Math.max(0, targetArmySize - totalFriendlyUnits)
        * difficultyProfile.production.settlerArmyShortfallWeight
        * gateStrength;
    const lowUtilizationPenalty = supplyUtilizationRatio < difficultyProfile.production.settlerUtilizationFloor
        ? (difficultyProfile.production.settlerUtilizationFloor - supplyUtilizationRatio) * difficultyProfile.production.settlerUtilizationPenaltyWeight * gateStrength
        : 0;
    const pressurePenalty = visibleEnemyPressure > 0
        ? (difficultyProfile.production.settlerVisibleEnemyBasePenalty
            + visibleEnemyPressure * difficultyProfile.production.settlerVisibleEnemyPerUnitPenalty) * gateStrength
        : 0;
    const reservePenalty = totalFriendlyUnits < reserveThreshold
        ? (reserveThreshold - totalFriendlyUnits) * difficultyProfile.production.settlerReservePenaltyWeight * gateStrength
        : 0;
    return (strategy.personality.scalars.defenseBias * 14 +
...
function scoreCatapultPreference(factionId, state, strategy, prototype) {
    if (!(prototype.tags ?? []).includes('siege'))
        return 0;
    const posture = strategy.posture;
    // Hill Clan strongly prefers catapults regardless of posture
    if (factionId === 'hill_clan') {
        const myUnitCount = Array.from(state.units.values()).filter((u) => u.factionId === factionId && u.hp > 0).length;
        const myCityCount = Array.from(state.cities.values()).filter((c) => c.factionId === factionId).length;
        if (myUnitCount >= 2 && myCityCount >= 1)
            return 6;
        if (myUnitCount >= 2)
            return 3;
    }
...
function scoreEnemyCounterPressure(enemyUnits, state, role) {
    let score = 0;
    for (const enemy of enemyUnits) {
        ...
        if (!enemyRole)
            continue;
        if (enemyRole === 'mounted' && role === 'melee')
            score += 1.5;
        if (enemyRole === 'ranged' && role === 'mounted')
            score += 1.25;
        if (enemyRole === 'melee' && role === 'ranged')
            score += 0.75;
        if (enemyRole === 'support' && role !== 'support')
            score += 0.5;
    }
    return score;
}
function scorePostureFit(posture, tags, role) {
    if (posture === 'recovery' || posture === 'defensive') {
        if (tags.includes('fortress') || role === 'melee')
            return 5;
        if (role === 'ranged')
            return 3;
        return 1;
...
function scoreIdentityFit(signatureUnit, economyAngle, prototype) {
    let score = 0;
    const signature = signatureUnit.toLowerCase();
    const name = prototype.name.toLowerCase();
    const tags = prototype.tags ?? [];
    if (signature.includes('cavalry') && prototype.derivedStats.role === 'mounted')
        score += 2;
    if (signature.includes('archer') && prototype.derivedStats.role === 'ranged')
        score += 2;
    if (signature.includes('catapult') && tags.includes('siege'))
        score += 2;
    if (signature.includes('elephant') && name.includes('elephant'))
        score += 2;
    if (signature.includes('immortal') && tags.includes('camel'))
        score += 2;
    if (signature.includes('bear') && name.includes('bear'))
        score += 2;
    if (signature.includes('galley') && tags.includes('naval'))
        score += 2;
    if (signature.includes('ship') && tags.includes('naval'))
        score += 2;
    if (economyAngle.includes('attritional') && tags.includes('poison'))
        score += 1.5;
    if (economyAngle.includes('mobile') && prototype.derivedStats.moves >= 3)
        score += 1.5;
    if (economyAngle.includes('siege') && prototype.derivedStats.role === 'melee')
        score += 1;
    if (economyAngle.includes('raiding') && tags.includes('naval'))
        score += 1.5;
    if (economyAngle.includes('coastal') && tags.includes('naval'))
        score += 1.5;
    if (economyAngle.includes('ocean') && tags.includes('transport'))
        score += 1.5;
    return score;
}
function scoreHybridFit(strategy, prototype) {
    if (!prototype.sourceRecipeId)
        return 0;
...
function scoreRecentCodifiedDomainPivot(domains, recentCodifiedDomains, scoringBonus) {
...
function scoreEmergentRuleCompletionFit(prototypeDomainIds, activeTripleStackDomains, difficultyProfile) {
...
function buildProductionReason(posture, role, cost, hybridScore, enemyCounterPressure, roleNeed, projectedSupplyMargin, codifiedPivotScore, settlerScore, underCapScore, armySizePressure, qualityLagScore) {
    if (settlerScore > 0) {
        return `${posture} settler expansion, village-funded growth, score ${settlerScore.toFixed(1)}`;
    }
    const parts = [`${posture} posture`, `${role} role`];
    if (roleNeed > 0.5)
        parts.push('fills role gap');
    if (enemyCounterPressure > 1)
        parts.push('counters enemy composition');
    if (hybridScore >= 3)
        parts.push('hybrid synergy payoff');
    if (codifiedPivotScore > 0)
        parts.push('recent codified domain pivot');
    if (underCapScore > 0)
        parts.push('under supply cap pressure');
    if (armySizePressure > 0)
        parts.push('army below target size');
    if (qualityLagScore > 0)
        parts.push('closes army quality gap');
