...
import { computeAiPersonalitySnapshot, createEmptyAiPersonalitySnapshot, scoreFocusTarget, scorePosture, shouldCommitAttack, } from './aiPersonality.js';
...
export function computeFactionStrategy(state, factionId, registry, difficulty) {
    ...
    const postureDecision = determinePosture(friendlyUnits.length, enemyUnits.length, threatenedCities, exhaustion, supplyDeficit, fronts, personality);
    ...
    const focusTargetDecision = chooseFocusTargets(state, factionId, primaryCityObjectiveId, fronts, posture, personality, difficultyProfile);
    ...
    const assignmentDecision = assignUnitIntents(state, factionId, friendlyUnits, posture, personality, threatenedCities, primaryCityObjectiveId, primaryFrontAnchor, focusTargetDecision.candidates, regroupAnchors, retreatAnchors, difficultyProfile);
    const debugReasons = buildDebugReasons(posture, threatenedCities, fronts, supplyDeficit, exhaustion, hybridGoal, absorptionGoal, postureDecision.reasons, focusTargetDecision.reasons, assignmentDecision.reasons);
    return {
        factionId,
        round: state.round,
        personality,
...
function createEmptyStrategy(round, factionId) {
    return {
        factionId,
        round,
...
function getLivingUnitsForFaction(state, factionId) {
    ...
    return Array.from(state.cities.values())
        ...
        for (const unit of state.units.values()) {
            ...
            if (unit.factionId === factionId) {
                ...
                if (!isUnitVisibleTo(state, factionId, unit))
                    continue;
...
function detectFronts(state, factionId, threatenedCities) {
    ...
    const registerFront = (friendlyPos, enemyPos, enemyFactionId, enemyCityId) => {
        const dist = hexDistance(friendlyPos, enemyPos);
        if (dist > FRONT_RADIUS * 2)
            return; // Last-seen fronts need larger radius allowance
        const anchor = dist <= 1
            ? friendlyPos
            : {
        ...
        const nearbyEnemyCity = enemyCityId
            ...
            : Array.from(state.cities.values()).find((city) => city.factionId === enemyFactionId && hexDistance(city.position, anchor) <= THREAT_RADIUS);
        if (!existing) {
            fronts.set(key, {
                anchor,
    ...
    for (const friendly of state.units.values()) {
        if (friendly.factionId !== factionId || friendly.hp <= 0)
            continue;
        for (const enemy of state.units.values()) {
            if (enemy.factionId === factionId || enemy.hp <= 0)
                continue;
            if (!isUnitVisibleTo(state, factionId, enemy))
                continue;
            ...
            const key = hexToKey(anchor);
            const existing = fronts.get(key);
            const nearbyEnemyCity = Array.from(state.cities.values()).find((city) => city.factionId === enemy.factionId && hexDistance(city.position, anchor) <= THREAT_RADIUS);
            if (!existing) {
                fronts.set(key, {
                    anchor,
    ...
    for (const threat of threatenedCities) {
        ...
        if (!city || !threat.nearestEnemyFactionId)
            continue;
    ...
    return Array.from(fronts.values()).sort((left, right) => right.pressure - left.pressure || hexToKey(left.anchor).localeCompare(hexToKey(right.anchor)));
}
function determinePosture(unitCount, enemyUnitCount, threatenedCities, exhaustion, supplyDeficit, fronts, personality) {
    ...
    const scoreByPosture = new Map();
    for (const posture of ['offensive', 'balanced', 'defensive', 'recovery', 'siege', 'exploration']) {
    ...
    const ranked = Array.from(scoreByPosture.entries()).sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]));
...
function choosePrimaryEnemyFaction(fronts, enemyUnits) {
    if (fronts.length > 0) {
        return fronts[0].enemyFactionId;
    }
    return enemyUnits[0]?.unit.factionId;
}
function choosePrimaryCityObjective(fronts, threatenedCities, posture, state, factionId) {
    ...
    const coastalFront = fronts.find((front) => {
        if (!front.enemyCityId || !state || !factionId)
            return false;
    ...
    if (state && factionId && posture !== 'defensive' && posture !== 'recovery') {
        ...
        if (lastSeenCities.length > 0) {
            return lastSeenCities[0].cityId;
        }
    }
    return undefined;
}
function choosePrimaryFrontAnchor(fronts, threatenedCities, posture) {
    if ((posture === 'defensive' || posture === 'recovery') && threatenedCities.length > 0) {
        return fronts[0]?.anchor;
    }
    return fronts[0]?.anchor;
}
function chooseFocusTargets(state, factionId, primaryCityObjectiveId, fronts, posture, personality, difficultyProfile) {
    ...
    const candidates = visibleEnemyUnits
        ...
        const antiSkirmishScore = isSkirmisherPrototype(targetPrototype)
            ...
            : 0;
        const personalityScore = scoreFocusTarget(personality, { targetDistance: cityDistance }, {
            isolated,
            capturable,
            wounded,
            isCity: false,
        ...
        const baseScore = sameFrontBonus + routedBonus + postureBonus + antiSkirmishScore + (1 - hpRatio) * 4 - cityDistance * 0.25;
        return {
    ...
    return {
        ...
        reasons: topCandidates.slice(0, 2).map((candidate, index) => `focus_${index + 1}=${candidate.unitId}:${candidate.score.toFixed(2)}(base=${candidate.baseScore.toFixed(2)},personality=${candidate.personalityScore.toFixed(2)})`),
    };
}
function buildRegroupAnchors(state, factionId, fronts, threatenedCities) {
    const anchors = [];
    if (fronts[0])
        anchors.push(fronts[0].anchor);
    if (fronts[1])
        anchors.push(fronts[1].anchor);
    for (const threat of threatenedCities) {
        const city = state.cities.get(threat.cityId);
        if (city)
            anchors.push(city.position);
    }
    return dedupeHexes(anchors);
}
function getFriendlyCityAnchors(state, factionId) {
    return dedupeHexes(Array.from(state.cities.values())
        .filter((city) => city.factionId === factionId)
 ...
 */
function findExplorationWaypoint(state, factionId, origin) {
...
function assignUnitIntents(state, factionId, friendlyUnits, posture, personality, threatenedCities, primaryCityObjectiveId, primaryFrontAnchor, focusTargetCandidates, regroupAnchors, retreatAnchors, difficultyProfile) {
    const intents = {};
    ...
    const focusTargetBudgets = buildSoftTargetBudgets(friendlyUnits.length, focusTargetCandidates, personality, difficultyProfile);
    const targetSelectionStats = {
        choices: 0,
        overfills: 0,
    };
    const squadPlan = buildStatelessSquadPlan(friendlyUnits, Math.max(1, Math.round(personality.thresholds.squadSize)), primaryFrontAnchor, regroupAnchors);
    for (const entry of friendlyUnits) {
        ...
        const selectedFocusTarget = selectFocusTargetCandidate(state, entry.unit, focusTargetBudgets, targetSelectionStats, difficultyProfile);
        let assignment = 'main_army';
        let waypointKind = 'front_anchor';
        ...
        if (faction && (entry.unit.learnedAbilities?.length ?? 0) > 0) {
            ...
            if (homeCity) {
                ...
                const shouldReturnToSacrifice = distToHome <= DISTANCE_THRESHOLD &&
                    !isElite &&
        ...
        if ((lowHp && !nearSiegeObjective) || (posture === 'recovery' && isolationScore > REGROUP_DISTANCE)) {
            ...
            const weightedChoice = chooseWeightedAssignment(personality, posture, {
                lowHp,
                fastUnit,
                isolationScore,
    ...
    const coordinatorReasons = applyDifficultyCoordinator(state, factionId, friendlyUnits, intents, posture, difficultyProfile);
    const learnLoopReasons = difficultyProfile.strategy.learnLoopEnabled
        ? applyDifficultyLearnAndSacrificeCoordinator(state, factionId, friendlyUnits, intents, getNearestEnemyCity(state, factionId, homeCity?.position
            ?? retreatAnchors[0]
            ?? friendlyUnits[0]?.unit.position
            ?? { q: 0, r: 0 }), difficultyProfile)
        : [];
    const waitForAlliesStats = applyWaitForAlliesGate(state, factionId, personality, intents, squadPlan, regroupAnchors);
    const summaryCounts = {};
    for (const intent of Object.values(intents)) {
        summaryCounts[intent.assignment] = (summaryCounts[intent.assignment] ?? 0) + 1;
    }
    const summary = ['main_army', 'raider', 'defender', 'siege_force', 'reserve', 'recovery', 'return_to_sacrifice']
...
function applyDifficultyCoordinator(state, factionId, friendlyUnits, intents, posture, difficultyProfile) {
    ...
    const garrisonUnit = [...activeArmy].sort((left, right) => {
        const distanceDelta = hexDistance(left.unit.position, homeCity.position) - hexDistance(right.unit.position, homeCity.position);
        if (distanceDelta !== 0) {
            return distanceDelta;
        }
        return compareUnitEntries(left, right);
    })[0];
    intents[garrisonUnit.unit.id] = buildHomeDefenseIntent(intents[garrisonUnit.unit.id], homeCity, `${coordinatorLabel} coordinator home garrison`);
    const economy = state.economy.get(factionId);
    const supplyRatio = economy && economy.supplyIncome > 0 ? economy.supplyDemand / economy.supplyIncome : 0;
    const idleNearHome = activeArmy.filter((entry) => entry.unit.id !== garrisonUnit.unit.id
        && entry.unit.status === 'ready'
        && hexDistance(entry.unit.position, homeCity.position) <= 3);
    if (supplyRatio < difficultyProfile.strategy.coordinatorMinSupplyRatio
    ...
    const hunterCount = Math.min(hunterPool.length, Math.max(difficultyProfile.strategy.coordinatorHunterFloor, Math.ceil(activeArmy.length * difficultyProfile.strategy.coordinatorHunterShare)));
    ...
    const assignHunterGroup = (huntersToAssign, assignedHunterIds, destinationCity, pushReason) => {
        const waypointVillage = posture === 'offensive' || posture === 'siege'
    ...
    if (difficultyProfile.strategy.multiAxisEnabled
        ...
        if (secondTargetCity) {
            ...
            if (primaryCount >= 2) {
                const rankHuntersForTarget = (pool, target) => [...pool].sort((left, right) => {
                ...
                const flankHunters = rankHuntersForTarget(hunterPool.filter((entry) => !primaryHunterIds.has(entry.unit.id)), secondTargetCity).slice(0, flankCount);
                if (flankHunters.length >= 2) {
                    const hunterIds = new Set();
                    const primaryWaypointVillage = assignHunterGroup(primaryHunters, hunterIds, targetCity, `${coordinatorLabel} coordinator hunter push`);
                    const flankWaypointVillage = assignHunterGroup(flankHunters, hunterIds, secondTargetCity, `${coordinatorLabel} coordinator flanking push`);
                    for (const defender of hunterPool) {
                        if (hunterIds.has(defender.unit.id)) {
                            continue;
                        }
                        intents[defender.unit.id] = buildHomeDefenseIntent(intents[defender.unit.id], homeCity, `${coordinatorLabel} coordinator home defense`);
                    }
                    return [
                        `${coordinatorLabel}_garrison=${garrisonUnit.unit.id}`,
    ...
    const hunterWaypointVillage = posture === 'offensive' || posture === 'siege'
...
function applyDifficultyLearnAndSacrificeCoordinator(state, factionId, friendlyUnits, intents, targetCity, difficultyProfile) {
    ...
    const maxReturnCount = Math.min(returnCandidates.length, Math.max(0, fieldArmy.length - minFieldForce), Math.max(1, Math.floor(fieldArmy.length * maxReturnShare)));
    ...
    const learnerTargetCity = difficultyProfile.strategy.learnLoopDomainTargetingEnabled
        ? getLearnLoopTargetCity(state, factionId, faction.learnedDomains, homeCity.position, fallbackTargetCity, difficultyProfile)
        : fallbackTargetCity;
...
function getLearnLoopTargetCity(state, factionId, learnedDomains, origin, fallbackTargetCity, difficultyProfile) {
    ...
    for (const city of state.cities.values()) {
        ...
        const adjustedDistance = distance
            - getEmergentRuleSacrificeDistanceCredit(knownDomains, cityFaction.nativeDomain, difficultyProfile);
...
function getRuleDomainGroups(rule) {
    if (rule.domainSets) {
...
function getEmergentRuleSacrificeDistanceCredit(knownDomains, candidateDomainId, difficultyProfile) {
...
function chooseVillageFirstHunterTarget(state, factionId, homePosition, targetCity, difficultyProfile) {
    ...
    return Array.from(state.villages.values())
        ...
        .filter(({ detour, distToCity }) => detour <= difficultyProfile.strategy.villageDetourTolerance
        && distToCity <= difficultyProfile.strategy.villageCityDistanceLimit)[0]?.village;
}
function buildHomeDefenseIntent(currentIntent, homeCity, reason) {
    return {
        assignment: 'defender',
        waypointKind: 'friendly_city',
        waypoint: homeCity.position,
...
function getNearestEnemyCity(state, factionId, origin) {
    let best;
    let bestDistance = Infinity;
    for (const city of state.cities.values()) {
        if (city.factionId === factionId) {
...
function getSecondNearestEnemyCity(state, factionId, origin, excludedCityId) {
...
function buildSoftTargetBudgets(unitCount, candidates, personality, difficultyProfile) {
    ...
    return candidates.map((candidate, index) => {
        ...
        const budget = Math.min(focusFireLimit, (weight / weightedSum) * assaultSlots + (index === 0 ? difficultyProfile.strategy.focusBudgetLeaderBonus : 0));
        return {
...
function selectFocusTargetCandidate(state, unit, budgets, stats, difficultyProfile) {
    if (budgets.length === 0)
        return undefined;
    let bestTarget;
    ...
    for (const budget of budgets) {
        ...
        const overfillPenalty = Math.max(0, budget.allocated - budget.budget) * difficultyProfile.strategy.focusOverfillPenalty;
...
function isSkirmisherPrototype(prototype) {
    if (!prototype)
        return false;
...
function buildStatelessSquadPlan(friendlyUnits, squadSize, primaryFrontAnchor, regroupAnchors) {
    const assignments = {};
...
function applyWaitForAlliesGate(state, factionId, personality, intents, squadPlan, regroupAnchors) {
    ...
    for (const [unitId, intent] of Object.entries(intents)) {
        if (!isAggressiveAssignment(intent.assignment))
            continue;
...
function isAggressiveAssignment(assignment) {
    return assignment === 'main_army' || assignment === 'raider' || assignment === 'siege_force';
}
function resolveIntentTarget(state, intent) {
...
function computeLocalEngagementSnapshot(state, factionId, unit, intent, target, intents, squadPlan) {
    ...
    for (const [otherId, otherIntent] of Object.entries(intents)) {
        const ally = state.units.get(otherId);
        if (!ally || ally.hp <= 0 || ally.factionId !== factionId)
            continue;
        if (!isAggressiveAssignment(otherIntent.assignment))
            continue;
        if (hexDistance(ally.position, target.hex) <= 2) {
            committedAllies += 1;
        }
        if (squadEntry && squadEntry.memberIds.includes(otherId) && hexDistance(ally.position, target.hex) <= 3) {
            squadSupport += 1;
        }
    }
    for (const enemy of state.units.values()) {
        if (enemy.hp <= 0 || enemy.factionId === factionId)
            continue;
...
function isExceptionalDoctrineOpportunity(personality, local) {
    ...
    if (!hasSkirmishDoctrine && !strongRaidIdentity)
        return false;
...
function chooseWeightedAssignment(personality, posture, context) {
    ...
    scoreByAssignment.set('siege_force', (scoreByAssignment.get('siege_force') ?? 0)
    ...
    const ranked = Array.from(scoreByAssignment.entries()).sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]));
    return {
        assignment: ranked[0][0],
        score: ranked[0][1],
    };
}
function nearestEnemySupportDistance(state, unit) {
    let nearest = Infinity;
    for (const other of state.units.values()) {
        if (other.id === unit.id || other.factionId !== unit.factionId || other.hp <= 0)
            continue;
        nearest = Math.min(nearest, hexDistance(unit.position, other.position));
    }
    return nearest === Infinity ? 99 : nearest;
}
function nearestFriendlyDistance(state, unit, factionId) {
    let nearest = Infinity;
    for (const other of state.units.values()) {
        if (other.id === unit.id || other.factionId !== factionId || other.hp <= 0)
            continue;
        nearest = Math.min(nearest, hexDistance(unit.position, other.position));
    }
    return nearest === Infinity ? 99 : nearest;
}
function nearestHex(origin, anchors) {
    let best = null;
    let bestDistance = Infinity;
 ...
 */
function findNearestCoastalEnemyObjective(state, factionId, origin) {
    let best;
...
function compareUnitEntries(left, right) {
    return compareUnits(left.unit, right.unit, undefined, left.prototype.name, right.prototype.name);
}
function compareUnits(left, right, state, leftName, rightName) {
    if (!left || !right)
        return 0;
...
function compareHexes(left, right) {
    if (!left || !right)
        return 0;
    return left.q - right.q || left.r - right.r;
}
function summarizePrimaryObjective(posture, threatenedCities, primaryCityObjectiveId, primaryEnemyFactionId) {
    if ((posture === 'defensive' || posture === 'recovery') && threatenedCities[0]) {
...
function buildDebugReasons(posture, threatenedCities, fronts, supplyDeficit, exhaustion, hybridGoal, absorptionGoal, postureReasons, focusReasons, assignmentReasons) {
    ...
    if (threatenedCities[0])
        reasons.push(`threatened_city=${threatenedCities[0].cityId}:${threatenedCities[0].threatScore}`);
    if (fronts[0])
        reasons.push(`front=${hexToKey(fronts[0].anchor)}:${fronts[0].pressure}`);
    if (supplyDeficit > 0)
        reasons.push(`supply_deficit=${supplyDeficit}`);
    if (exhaustion > 0)
        reasons.push(`war_exhaustion=${exhaustion}`);
    if (hybridGoal.preferredRecipeIds[0])
        reasons.push(`hybrid=${hybridGoal.preferredRecipeIds[0]}`);
    if (absorptionGoal.targetFactionId)
        reasons.push(`absorption_target=${absorptionGoal.targetFactionId}`);
    return reasons;
}
function buildHybridGoal(state, factionId, registry) {
    const faction = state.factions.get(factionId);
    if (!faction) {
    ...
    const recipes = registry.getAllHybridRecipes()
        ...
        .map((recipe) => {
        const synergyScore = recipe.tags?.reduce((sum, tag) => sum + (faction.learnedDomains.includes(tag) ? 1 : 0), 0) ?? 0;
...
function buildAbsorptionGoal(state, factionId, enemyUnits, primaryEnemyFactionId) {
    const faction = state.factions.get(factionId);
    if (!faction) {
    ...
    const weakEnemyFactionCounts = new Map();
    for (const unit of state.units.values()) {
        if (unit.hp <= 0)
            continue;
        if (unit.factionId === factionId)
    ...
    const weakEnemyTarget = Array.from(weakEnemyFactionCounts.entries())
        .sort((left, right) => left[1] - right[1] || String(left[0] ?? '').localeCompare(String(right[0] ?? '')))[0]?.[0];
    return {
...
export function getUnitIntent(strategy, unitId) {
    return strategy?.unitIntents[unitId];
}
export function scoreStrategicTerrain(state, factionId, hex) {
    const faction = state.factions.get(factionId);
    const terrainId = state.map?.tiles.get(hexToKey(hex))?.terrain ?? 'plains';
    return getTerrainPreferenceScore(faction, terrainId);
}
export function getNearbySupportScore(state, factionId, hex) {
    let support = 0;
    for (const unit of state.units.values()) {
...
export function isThreatenedCityHex(state, strategy, hex) {
    if (!strategy)
        return false;
...
export function getNearestFriendlyCity(state, factionId, origin) {
    let best;
    let bestDist = Infinity;
    for (const city of state.cities.values()) {
        if (city.factionId !== factionId)
