...
const DOMAINS = abilityDomainsData.domains;
...
const EXPOSURE_THRESHOLDS = [100, 150, 200];
...
const PROTOTYPE_COST_MODIFIERS = {
 ...
 */
export function getNextExposureThreshold(learnedDomainsCount, nativeDomain) {
 ...
 */
export function getDomainIdByTag(tag) {
 ...
 */
export function getDomainIdsByTags(tags) {
 ...
 */
export function isForeignDomain(faction, domainId) {
 ...
 */
export function gainExposure(state, factionId, domainId, amount, trace) {
    ...
    if (isNowLearned && !wasLearned) {
        ...
        for (const existingDomain of faction.learnedDomains) {
            ...
            const synergyExists = pairSynergiesData.pairSynergies.some((pair) => pair.id === synergyPairId || pair.id === reversePairId);
            if (synergyExists) {
                if (trace) {
 ...
 */
export function checkDomainLearned(faction) {
    if (faction.learnedDomains.length >= MAX_LEARNED_DOMAINS) {
        return null;
    }
    const threshold = getNextExposureThreshold(faction.learnedDomains.length, faction.nativeDomain);
 ...
 */
export function getPrototypeCostModifier(faction, domainId) {
 ...
 */
export function incrementPrototypeMastery(state, factionId, domainId) {
    const faction = state.factions.get(factionId);
    if (!faction)
        return state;
 ...
 */
export function getExposedDomains(faction) {
    return Object.keys(faction.exposureProgress).filter(domainId => isForeignDomain(faction, domainId));
}
/**
 * Get exposure progress details for a specific domain.
 */
export function getExposureDetails(faction, domainId) {
    if (!isForeignDomain(faction, domainId)) {
        return null;
    }
 ...
 */
export function calculatePrototypeCost(baseCost, faction, domainIds) {
    let maxModifier = 1.0;
