 ...
 */
export function getCaptureRampMultiplier(turnsSinceCapture) {
    if (turnsSinceCapture === undefined || turnsSinceCapture === null)
        return 1;
    if (turnsSinceCapture <= CAPTURE_RAMP_TURNS)
        return 0;
 ...
 */
export function advanceCaptureTimers(state, factionId) {
    ...
    for (const cityId of faction.cityIds) {
        ...
        if (!city || city.turnsSinceCapture === undefined)
            continue;
 ...
 */
export function deriveResourceIncome(state, factionId, registry) {
    const faction = state.factions.get(factionId);
    if (!faction) {
 ...
 */
export function getSupplyDeficit(economy) {
