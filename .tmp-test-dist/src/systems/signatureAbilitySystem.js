...
export function getAbilityDomains() {
    return abilityDomainsData.domains;
}
/** Find the ability domain that matches a given tag */
export function getDomainForTag(tag) {
...
export function unitHasTag(unit, state, tag) {
 ...
 */
export function isElephantUnit(prototype) {
 ...
 */
export function hasFortressTraining(unitId, state, registry) {
    const prototype = state.prototypes.get(unitId);
    if (!prototype)
        return false;
    // Check prototype tags first
    if (prototype.tags?.includes('fortress'))
        return true;
    // Also check component tags for backward compatibility
    const componentIds = prototype.componentIds ?? [];
    for (const componentId of componentIds) {
        const component = registry.getComponent(componentId);
        if (component?.tags?.includes('fortress'))
            return true;
    }
    return false;
}
 ...
 */
export function isOnTundra(state, unit) {
 ...
 */
export function isLandTerrain(terrainId) {
 ...
 */
export function getBulwarkDefenseBonus(defender, state, registry, baseBonus = 0) {
    const fortressDomain = getDomainForTag('fortress');
    const bonusValue = fortressDomain
        ? fortressDomain.baseEffect.value ?? baseBonus
        : baseBonus;
 ...
 */
export function applyKnockback(state, attacker, defender, distance = 1) {
    const neighbors = getNeighbors(defender.position);
    ...
    for (const hex of neighbors) {
        ...
        const tile = state.map?.tiles.get(`${hex.q},${hex.r}`);
        if (!tile)
            continue;
        ...
        if (dirQ !== 0 && Math.sign(hexDirQ) === Math.sign(dirQ))
            score += 2;
        if (dirR !== 0 && Math.sign(hexDirR) === Math.sign(dirR))
            score += 2;
        if (score > bestScore) {
            bestScore = score;
            bestHex = hex;
 ...
 */
export function applyStrikeFirst(attacker, defender, isCharge, defenderDestroyed) {
    if (!isCharge)
        return false;
 ...
 */
export function applyPoisonDoT(target, stacks, damagePerTurn, duration) {
 ...
 */
export function resolveStealth(unit, prototypeTags) {
    ...
    if (!prototypeTags.includes('stealth'))
        return false;
 ...
 */
export function tickStealthCooldown(unit) {
    if (unit.turnsSinceStealthBreak > 0) {
 ...
 */
export function enterStealth(unit, prototypeTags) {
    if (!prototypeTags.includes('stealth'))
        return unit;
    if (unit.turnsSinceStealthBreak > 0)
        return unit;
 ...
 */
export function getNatureHealingAura() {
    const healingDomain = getDomainForTag('healing') ?? getDomainForTag('druid');
    if (!healingDomain)
        return { selfHeal: 2, allyHeal: 1 };
    return {
        selfHeal: healingDomain.baseEffect.selfHeal ?? 2,
        allyHeal: healingDomain.baseEffect.allyHeal ?? 1,
    };
}
/**
 * getTidalCoastDebuff — return the defense debuff from tidal_warfare domain.
 */
export function getTidalCoastDebuff() {
    const tidalDomain = getDomainForTag('naval');
    if (!tidalDomain)
        return 0.25;
    return tidalDomain.baseEffect.coastDebuff ?? 0.25;
}
/**
 * getStampedeDistance — return knockback distance from charge domain.
 */
export function getStampedeDistance() {
    const stampedeDomain = getDomainForTag('elephant');
    if (!stampedeDomain)
        return 1;
    return stampedeDomain.baseEffect.distance ?? 1;
}
