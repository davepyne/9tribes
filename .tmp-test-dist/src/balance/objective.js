...
export function scoreBalanceSummary(summary) {
    ...
    const nonViableFactions = factionMetrics.filter((f) => f.avgLivingUnits < 2 && f.avgCities < 0.3).length;
    const survivalPenalty = nonViableFactions * 2.5;
    // 2. Near-death penalty: factions barely hanging on but not totally dead
    const nearDeathFactions = factionMetrics.filter((f) => f.avgLivingUnits >= 2 && f.avgLivingUnits < 4 && f.avgCities < 0.5).length;
    ...
    const inactivityPenalty = (avgBattlesPerSeed < 8 ? (8 - avgBattlesPerSeed) * 0.6 : 0) +
    ...
    const unresolvedPenalty = (unresolvedRate > 0.8 ? (unresolvedRate - 0.8) * 25 : 0) +
    ...
    const totalLivingUnits = factionMetrics.reduce((sum, f) => sum + Object.values(f.avgUnitComposition.byRole).reduce((s, v) => s + v, 0), 0);
    const avgSiegeUnits = factionMetrics.reduce((sum, f) => sum + (f.avgUnitComposition.byChassis['catapult_frame'] ?? 0), 0) / factionMetrics.length;
