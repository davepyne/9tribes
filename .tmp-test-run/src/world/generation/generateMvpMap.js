 ...
 */
export function generateMvpMap(rng, width = 40, height = 30) {
    ...
    for (let i = 0; i < clusterCount; i++) {
        ...
        for (let q = Math.max(0, centerQ - radius); q <= Math.min(map.width - 1, centerQ + radius); q++) {
            for (let r = Math.max(0, centerR - radius); r <= Math.min(map.height - 1, centerR + radius); r++) {
                ...
                if (!tile)
                    continue;
                if (tile.terrain === 'river' || tile.terrain === 'coast' || tile.terrain === 'ocean')
                    continue;
 ...
 */
function ensureJungleNearStart(map, startHex, radius) {
        ...
        if (tile.terrain !== 'river' && tile.terrain !== 'coast') {
            ...
            for (let dq = -1; dq <= 1; dq++) {
                for (let dr = -1; dr <= 1; dr++) {
                    ...
                    if (!ntile)
                        continue;
                    if (ntile.terrain === 'river' || ntile.terrain === 'coast' || ntile.terrain === 'ocean')
                        continue;
...
function carveSavannahs(map, rng) {
 ...
 */
function ensureHillsNearStart(map, startHex, radius) {
 ...
 */
function ensureSavannahNearStart(map, startHex, radius) {
 ...
 */
function ensureRiverNearStart(map, startHex, radius) {
 ...
 */
function ensureCoastNearStart(map, startHex, radius) {
    ...
    for (const neighbor of getNeighbors(startHex)) {
        ...
        if (!tile)
            continue;
        if (tile.terrain === 'river' || tile.terrain === 'ocean')
            continue; // preserve water
        tile.terrain = 'coast';
    }
}
