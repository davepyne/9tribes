 ...
 */
function extractTier(nodeId) {
    const match = nodeId.match(/_t(\d+)$/);
    return match ? parseInt(match[1], 10) : 1;
}
function getEffectiveXpCost(_faction, xpCost) {
    return xpCost;
}
/**
 ...
 */
function getCandidateNodes(faction, completedNodes, registry) {
 ...
 */
function scoreNativePriority(candidate) {
 ...
 */
function scorePosture(posture, domainId) {
 ...
 */
function scoreSignatureDomain(signatureUnit, domainId) {
 ...
 */
function scoreSynergy(strategy, codifies) {
    let score = 0;
    for (const domainId of codifies) {
        if (strategy.hybridGoal.desiredDomainIds.includes(domainId))
            score += 2.5;
    }
    return score;
}
 ...
 */
function scoreTierUrgency(tier) {
 ...
 */
function scoreCostEfficiency(faction, xpCost) {
 ...
 */
function scoreImmediateUnlocks(state, factionId, nodeId, registry) {
    void state;
    void factionId;
    void nodeId;
    void registry;
 ...
 */
function scoreKnowledgeBonus(faction, xpCost) {
    void faction;
    void xpCost;
    return 0;
}
 ...
 */
function scoreGameStateUrgency(faction, posture, domainId) {
...
function scoreDoctrinePackageCompletion(candidate, completedNodes, strategy) {
...
function getDomainsWithResearchProgress(research) {
...
function scoreNormalTier3DepthFocus(candidate, domainsWithProgress, difficultyProfile) {
    if (!difficultyProfile.adaptiveAi)
        return 0;
    if (candidate.tier !== 3)
        return 0;
    if (!domainsWithProgress.has(candidate.domainId))
        return 0;
    return difficultyProfile.research.tier3DepthWeight;
}
function scoreNormalBreadthPivot(candidate, faction, progression, domainsWithProgress, difficultyProfile) {
    if (!difficultyProfile.adaptiveAi)
        return 0;
...
function scoreNormalHybridBreadth(candidate, strategy, progression, difficultyProfile) {
    if (!difficultyProfile.adaptiveAi)
        return 0;
    if (candidate.tier !== 2)
        return 0;
    if (!strategy.hybridGoal.desiredDomainIds.includes(candidate.domainId))
        return 0;
...
function scoreNormalEmergentBreadth(candidate, progression, difficultyProfile) {
    if (!difficultyProfile.adaptiveAi)
        return 0;
    if (candidate.tier !== 2)
        return 0;
    if (progression.emergentEligibleDomains.includes(candidate.domainId))
        return 0;
    if (progression.t2Domains.length >= 2)
        return 0;
...
function getReachableTripleStackOpportunities(codifiedDomains, unlockedDomains) {
    const rules = emergentRulesData.rules.filter((rule) => rule.condition !== 'default');
    const opportunities = [];
    for (const rule of rules) {
        ...
        if (groups.length === 1) {
            ...
            const missingDomains = new Set(eligibleDomains.filter((domainId) => unlockedDomains.has(domainId) && !codifiedDomains.has(domainId)));
...
function scoreNormalTripleStackFocus(candidate, opportunities, difficultyProfile) {
    if (!difficultyProfile.adaptiveAi)
        return 0;
    let score = 0;
...
function scoreLogisticsFit(state, factionId, strategy, candidate) {
    ...
    let score = 0;
    if (mountedShare >= 0.4) {
        if (candidate.domainId === 'hitrun')
            score += 1.5;
        if (candidate.domainId === 'nature_healing')
            score += 1.2;
        if (candidate.domainId === 'river_stealth')
            score += 1.0;
        if (candidate.domainId === 'fortress')
            score += 1.0;
    }
    if (candidate.domainId === 'charge' || candidate.domainId === 'heavy_hitter') {
        score -= 0.8;
    }
    const personalityPreference = scoreResearchCandidate(strategy.personality, { supplyDeficit }, { domainId: candidate.domainId, codifies: candidate.def.codifies });
    return score + personalityPreference * 0.8;
}
// ---------------------------------------------------------------------------
// Reason string builder
// ---------------------------------------------------------------------------
function buildResearchReason(candidate, posture, scores) {
    const parts = [`${posture} posture`, candidate.def.id];
    if (candidate.isNative)
        parts.push('native domain');
    if (scores.native > 0)
        parts.push('native priority');
    if (scores.posture > 0)
        parts.push('posture-aligned');
    if (scores.signature > 0)
        parts.push('signature unit synergy');
    if (scores.synergy > 0)
        parts.push('hybrid/absorption goal');
    if (scores.tier > 0)
        parts.push('tier 2 urgency');
    if (scores.immediate > 0)
        parts.push('unlocks production');
    if (scores.knowledge > 0)
        parts.push('cosmopolitan knowledge bonus');
    if (scores.gameState > 0)
        parts.push('game-state urgency');
    if (scores.doctrinePackage > 0)
        parts.push('doctrine package');
    if (scores.logistics > 0)
        parts.push('logistics fit');
    if (scores.depthFocus > 0)
        parts.push('tier 3 depth push');
    if (scores.breadthPivot > 0)
        parts.push('breadth pivot');
    if (scores.hybridBreadth > 0)
        parts.push('hybrid breadth');
    if (scores.emergentBreadth > 0)
        parts.push('emergent breadth');
    if (scores.tripleStack > 0)
        parts.push('triple-stack reach');
    return parts.join(', ');
}
// ---------------------------------------------------------------------------
// Public API
 ...
 */
export function rankResearchPriorities(state, factionId, strategy, registry, difficulty) {
    const difficultyProfile = getAiDifficultyProfile(difficulty);
    const faction = state.factions.get(factionId);
    const research = state.research.get(factionId);
    if (!faction || !research)
        return [];
    const candidates = getCandidateNodes(faction, research.completedNodes, registry);
    const domainsWithProgress = getDomainsWithResearchProgress(research);
    const progression = getDomainProgression(faction, research);
    const tripleStackOpportunities = getReachableTripleStackOpportunities(new Set(progression.emergentEligibleDomains), new Set(faction.learnedDomains));
    return candidates
        .map((candidate) => {
        const native = scoreNativePriority(candidate);
        const postureScore = scorePosture(strategy.posture, candidate.domainId);
        const signatureScore = scoreSignatureDomain(faction.identityProfile.signatureUnit, candidate.domainId);
        ...
        const immediateScore = scoreImmediateUnlocks(state, factionId, candidate.def.id, registry);
        ...
        const doctrinePackageScore = scoreDoctrinePackageCompletion(candidate, research.completedNodes, strategy);
        ...
        const breadthPivotScore = scoreNormalBreadthPivot(candidate, faction, progression, domainsWithProgress, difficultyProfile);
 ...
 */
export function chooseStrategicResearch(state, factionId, strategy, registry, difficulty) {
    const difficultyProfile = getAiDifficultyProfile(difficulty);
