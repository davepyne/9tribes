...
let sacrificeSynergyEngine = null;
function getSacrificeSynergyEngine() {
    if (!sacrificeSynergyEngine) {
        sacrificeSynergyEngine = new SynergyEngine(pairSynergiesData.pairSynergies, emergentRulesData.rules, Object.values(abilityDomainsData.domains));
    }
 ...
 */
export function canSacrifice(unit, faction, state) {
 ...
 */
export function performSacrifice(unitId, factionId, state, registry, trace) {
    ...
    const newlyUnlockedDomains = learnedDomains.filter((domainId, index) => !faction.learnedDomains.includes(domainId) && learnedDomains.indexOf(domainId) === index);
    ...
    const progression = getDomainProgression({ nativeDomain: updatedFaction.nativeDomain, learnedDomains: newLearnedDomains }, refreshedResearch);
    const tripleStack = getSacrificeSynergyEngine().resolveFactionTriple(progression.pairEligibleDomains, progression.emergentEligibleDomains);
 ...
 */
function removeUnit(state, unitId, factionId) {
 ...
 */
function autoCompleteResearchForDomains(state, factionId, domainIds, registry, trace) {
        ...
        for (const domain of allDomains) {
            for (const [nodeId, node] of Object.entries(domain.nodes)) {
                ...
                if (node.codifies?.includes(domainId)) {
                    const researchNodeId = nodeId;
                    // If this node's prerequisites are met, complete it
                    const prerequisitesMet = (node.prerequisites ?? []).every((prereqId) => completedNodesSet.has(prereqId));
    ...
    const completedNodes = Array.from(completedNodesSet);
    const researchMap = new Map(state.research);
    researchMap.set(factionId, {
        ...currentResearch,
        completedNodes,
        // Clear active node if it was completed
        activeNodeId: completedNodes.includes(currentResearch.activeNodeId) ? null : currentResearch.activeNodeId,
    });
 ...
 */
function getUnitName(unit, state) {
