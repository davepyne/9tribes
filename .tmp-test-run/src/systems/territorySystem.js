 ...
 */
export function getCityTerritoryHexes(city, map, radius = 2) {
 ...
 */
export function isHexContested(hex, claimingFactionId, state) {
 ...
 */
export function getHexOwner(hex, state) {
 ...
 */
export function calculateTerritoryYield(factionId, state) {
    let totalYield = 0;
    const countedHexes = new Set();
    for (const [, city] of state.cities) {
 ...
 */
export function isCityEncircled(city, state) {
    let enemyCount = 0;
    for (const [, unit] of state.units) {
 ...
 */
export function isEncirclementBroken(city, state) {
    let enemyCount = 0;
    for (const [, unit] of state.units) {
        if (unit.factionId === city.factionId || unit.hp <= 0)
            continue;
...
function getUnitAtHexFromState(state, hex) {
