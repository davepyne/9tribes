 ...
 */
export function isTransportUnit(prototype, registry) {
    const chassis = registry.getChassis(prototype.chassisId);
    if (!chassis)
        return false;
    if (!chassis.tags?.includes('transport'))
        return false;
    if (!chassis.transportCapacity || chassis.transportCapacity <= 0)
        return false;
    return true;
}
/**
 ...
 */
export function getTransportCapacity(chassisId, registry) {
 ...
 */
export function getEmbarkedCount(transportId, transportMap) {
 ...
 */
export function getEmbarkedUnits(transportId, transportMap) {
    return transportMap.get(transportId)?.embarkedUnitIds ?? [];
}
/**
 * Check if a unit is currently embarked
 */
export function isUnitEmbarked(unitId, transportMap) {
 ...
 */
export function getUnitTransport(unitId, transportMap) {
    for (const transportState of transportMap.values()) {
        if (transportState.embarkedUnitIds.includes(unitId)) {
 ...
 */
function isValidDisembarkHex(state, hex) {
    if (!state.map)
        return false;
 ...
 */
export function canBoardTransport(state, landUnitId, transportId, registry, transportMap) {
    // Get the land unit
    ...
    if (landUnit.factionId !== transport.factionId)
        return false;
    // Land unit must be adjacent to transport
    if (hexDistance(landUnit.position, transport.position) !== 1)
        return false;
    ...
    if (!prototype)
        return false;
    const chassis = registry.getChassis(prototype.chassisId);
    if (!chassis || !chassis.tags?.includes('transport'))
        return false;
 ...
 */
export function boardTransport(state, landUnitId, transportId, transportMap) {
    // Get existing transport state or create new
    let transportState = transportMap.get(transportId);
    const newEmbarkedUnitIds = transportState
 ...
 */
export function canDisembark(state, transportId, targetHex, registry, transportMap) {
    // Get the transport
    const transport = state.units.get(transportId);
    if (!transport)
        return false;
    ...
    if (hexDistance(transport.position, targetHex) !== 1)
        return false;
    // Target hex must be a valid land hex
    if (!isValidDisembarkHex(state, targetHex))
        return false;
 ...
 */
export function disembarkUnit(state, transportId, unitId, targetHex, registry, transportMap) {
 ...
 */
export function updateEmbarkedPositions(state, transportId, newPosition, transportMap) {
    const transportState = transportMap.get(transportId);
    if (!transportState)
        return state;
    const newUnits = new Map(state.units);
 ...
 */
export function destroyTransport(state, transportId, transportMap) {
    const transportState = transportMap.get(transportId);
    if (!transportState)
        return { state, transportMap };
 ...
 */
export function getValidDisembarkHexes(state, transportId, registry, transportMap) {
    const transport = state.units.get(transportId);
