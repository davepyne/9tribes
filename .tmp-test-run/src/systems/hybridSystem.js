...
export function unlockHybridRecipes(game, factionId, registry) {
    ...
    for (const recipe of registry.getAllHybridRecipes()) {
        ...
        const prototypeId = createPrototypeId(`${factionId}_${recipe.id}`);
        let prototype;
        try {
            prototype = assemblePrototype(factionId, recipe.chassisId, recipe.componentIds, registry, Array.from(current.prototypes.keys()), {
