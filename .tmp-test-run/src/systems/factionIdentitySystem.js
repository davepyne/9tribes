...
export function isPoorTerrain(terrainId) {
    return terrainId ? POOR_TERRAINS.has(terrainId) : false;
}
export function getFactionForUnit(state, unit) {
    return state.factions.get(unit.factionId);
}
const ROUGH_TERRAINS = new Set(['forest', 'jungle', 'hill', 'tundra', 'desert']);
export function getHealingBonus(faction, terrainId) {
    const passive = faction?.identityProfile.passiveTrait;
    if (passive === 'healing_druids') {
        if (terrainId === 'forest')
            return 2;
        if (ROUGH_TERRAINS.has(terrainId))
            return 2;
        return 1;
    }
    return 0;
}
export function getMovementCostModifier(faction, originTerrainId, targetTerrainId) {
    const passive = faction?.identityProfile.passiveTrait;
...
export function getCombatAttackModifier(faction, attackerTerrain, defenderTerrain) {
    const passive = faction?.identityProfile.passiveTrait;
...
export function getCombatDefenseModifier(faction, defenderTerrain) {
    const passive = faction?.identityProfile.passiveTrait;
    ...
    if (passive === 'greedy') {
        if (terrainId === 'coast' || terrainId === 'ocean')
            return 0.15;
        return 0.05; // pirate grit — always a little tougher
    }
    return 0;
}
export function getEconomyProductionBonus(faction, terrainId) {
    const passive = faction?.identityProfile.passiveTrait;
...
export function getEconomySupplyBonus(faction, terrainId) {
    const passive = faction?.identityProfile.passiveTrait;
    if (passive === 'desert_logistics' && terrainId === 'desert') {
        return 0.10;
    }
...
export function isUnitRiverStealthed(faction, terrainId) {
    const passive = faction?.identityProfile.passiveTrait;
    return passive === 'river_assault' && isWaterTerrain(terrainId);
}
export function getTerrainPreferenceScore(faction, terrainId) {
    if (!faction) {
        return 0;
    }
...
const DEFAULT_DESERT_SWARM_CONFIG = {
    threshold: 3,
    attackBonus: 1,
    defenseMultiplier: 1.10,
};
export function getDesertSwarmBonus(faction, unit, state, config = DEFAULT_DESERT_SWARM_CONFIG) {
