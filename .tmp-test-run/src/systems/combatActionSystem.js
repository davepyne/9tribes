...
import { getCombatAttackModifier, getCombatDefenseModifier, getDesertSwarmBonus, } from './factionIdentitySystem.js';
import { getVeteranDefenseBonus, getVeteranMoraleBonus, getVeteranStatBonus, resolveCombat, } from './combatSystem.js';
...
import { applyKnockback, applyPoisonDoT, enterStealth, findRetreatHex, getBulwarkDefenseBonus, getTidalCoastDebuff, } from './signatureAbilitySystem.js';
...
import { recordBattleFought, recordEnemyKilled, recordPromotion, updateCombatRecordOnElimination, updateCombatRecordOnLoss, updateCombatRecordOnWin, } from './historySystem.js';
import { getUnitAtHex } from './occupancySystem.js';
import { getWallDefenseBonus } from './siegeSystem.js';
import { applyCombatSynergies, } from './synergyEffects.js';
import { calculateSynergyAttackBonus, calculateSynergyDefenseBonus, getSynergyEngine, } from './synergyRuntime.js';
const WATER_TERRAIN = new Set(['coast', 'river', 'ocean']);
function formatPercent(value) {
    return `${value > 0 ? '+' : ''}${Math.round(value * 100)}%`;
}
function humanizeCombatEffect(effect) {
...
function pushCombatEffect(effects, label, detail, category) {
    effects.push({ label, detail, category });
}
function getImprovementBonus(state, position) {
...
function removeDeadUnitsFromFactions(factions, units) {
    const nextFactions = new Map(factions);
    for (const [factionId, faction] of nextFactions.entries()) {
        nextFactions.set(factionId, {
            ...faction,
            unitIds: faction.unitIds.filter((unitId) => units.has(unitId)),
        });
    }
    return nextFactions;
}
function canAttackTarget(state, registry, attacker, defender) {
...
function writeUnitToState(state, unit) {
    if (!unit) {
        return state;
    }
...
function applyKnockbackDistance(state, attackerId, defenderId, distance) {
    let current = state;
...
function createCombatActionPreviewRecord(state, attackerId, defenderId, result, triggeredEffects, braceTriggered, attackerWasStealthed, details) {
    const attacker = state.units.get(attackerId);
    const defender = state.units.get(defenderId);
    const attackerPrototype = attacker ? state.prototypes.get(attacker.prototypeId) : null;
    const defenderPrototype = defender ? state.prototypes.get(defender.prototypeId) : null;
...
function maybeAbsorbFaction(state, victorFactionId, defeatedFactionId) {
    const stillAlive = Array.from(state.units.values()).some((unit) => unit.factionId === defeatedFactionId && unit.hp > 0);
    if (stillAlive) {
        return state;
    }
...
export function previewCombatAction(state, registry, attackerId, defenderId) {
    const attacker = state.units.get(attackerId);
    const defender = state.units.get(defenderId);
    ...
    const attackerPrototype = state.prototypes.get(attacker.prototypeId);
    const defenderPrototype = state.prototypes.get(defender.prototypeId);
    ...
    const defenderDoctrine = defenderFaction
        ...
        : undefined;
    const canChargeAttack = (attackerPrototype.derivedStats.range ?? 1) <= 1
    ...
    const defenderOnCity = Array.from(state.cities.values()).some((city) => city.position.q === defender.position.q && city.position.r === defender.position.r);
    ...
    if (defenderDoctrine?.shieldWallEnabled) {
        ...
        if (attackerIsRanged && defenderIsInfantry) {
            ...
            const hasSupportAlly = Array.from(state.units.values()).some((neighbor) => neighbor.id !== defender.id
    ...
    const resolveSynergies = (unit, prototype, faction, enemyUnit, enemyPrototype) => {
    ...
    const wallDefenseBonus = getWallDefenseBonus(state, defender.position, registry.getSignatureAbility('coral_people')?.wallDefenseMultiplier ?? 2);
    const result = resolveCombat(attacker, defender, attackerPrototype, defenderPrototype, getVeteranStatBonus(registry, attacker.veteranLevel), getVeteranDefenseBonus(registry, defender.veteranLevel), attackerTerrain, defenderTerrain, improvementDefenseBonus + wallDefenseBonus, getVeteranMoraleBonus(registry, defender.veteranLevel), registry, flankingBonus, situationalAttackModifier + chargeAttackBonus, situationalDefenseModifier, state.rngState, rearAttackBonus, braceDefenseBonus, ambushAttackBonus, (rearAttackBonus > 0 ? 18 : 0) + (attacker.preparedAbility === 'ambush' ? 10 : 0), retaliationDamageMultiplier, 0, isChargeAttack, attackerWasStealthed, attackerSynergyResult.chargeShield, defenderSynergyResult.antiDisplacement || defenderDoctrine?.armorPenetrationEnabled === true, attackerSynergyResult.stealthChargeMultiplier, attackerSynergyResult.sandstormAccuracyDebuff, forestFirstStrike);
    ...
    if (result.roleModifier !== 0) {
        ...
        pushCombatEffect(triggeredEffects, 'Role Effectiveness', `${attackerPrototype.derivedStats.role ?? 'unknown'} vs ${defenderPrototype.derivedStats.role ?? 'unknown'}: ${sign}${(result.roleModifier * 100).toFixed(0)}%`, 'positioning');
    }
    ...
    return createCombatActionPreviewRecord(state, attackerId, defenderId, result, triggeredEffects, braceTriggered, attackerWasStealthed, {
        attackerTerrainId,
        defenderTerrainId,
        isChargeAttack,
        chargeAttackBonus,
...
export function applyCombatAction(state, registry, preview) {
    ...
    if (preview.result.defenderDestroyed
        ...
        const captureResult = attemptCapture(current, nextAttacker, defender, registry, autoCaptureAbility
            ?? (isGreedyCoastal && !hasCaptureAbility(attackerPrototype, registry)
                ? registry.getSignatureAbility(attacker.factionId)
                : null), current.rngState);
    ...
    if (!preview.result.defenderDestroyed && preview.result.defenderFled && nextAttacker.hp > 0 && attackerDoctrine?.captureRetreatEnabled) {
        const retreatCapture = attemptNonCombatCapture(current, preview.attackerId, preview.defenderId, registry, 0.15, 0.25, 0, current.rngState);
    ...
    if (preview.result.defenderDestroyed && !capturedOnKill) {
        ...
        const proto = current.prototypes.get(destroyedDefender.prototypeId);
    ...
    if (preview.result.attackerDestroyed) {
        ...
        const proto = current.prototypes.get(destroyedAttacker.prototypeId);
        if (proto && isTransportUnit(proto, registry)) {
    ...
    current = maybeAbsorbFaction(current, attacker.factionId, defender.factionId);
    ...
    if (preview.result.defenderDestroyed && attackerWarExhaustion) {
        current = {
            ...current,
            warExhaustion: new Map(current.warExhaustion).set(attacker.factionId, addExhaustion(attackerWarExhaustion, EXHAUSTION_CONFIG.UNIT_KILLED)),
        };
    }
    if (preview.result.attackerDestroyed && defenderWarExhaustion) {
        current = {
            ...current,
            warExhaustion: new Map(current.warExhaustion).set(defender.factionId, addExhaustion(defenderWarExhaustion, EXHAUSTION_CONFIG.UNIT_KILLED)),
        };
    }
    const hitAndRunEligible = attackerDoctrine?.universalHitAndRunEnabled
    ...
    if (updatedAttacker) {
        updatedAttacker = recordBattleFought(updatedAttacker, defender.id, preview.result.defenderDestroyed, preview.result.attackerDamage, preview.result.defenderDamage);
        if (preview.result.defenderDestroyed) {
    ...
    if (!preview.result.defenderDestroyed && preview.result.defenderDamage > 0 && canInflictPoison && updatedDefender) {
        ...
        updatedDefender = applyPoisonDoT(updatedDefender, (attackerDoctrine?.poisonStacksOnHit ?? 1) + extraStacks, attackerDoctrine?.poisonDamagePerStack ?? 3, 3);
        updatedDefender = { ...updatedDefender, poisonedBy: attacker.factionId };
