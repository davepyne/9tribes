...
function mergeDoctrineWeights(target, weights, factor) {
    if (!weights)
        return;
...
function buildTerrainScores(baseline) {
    const terrainScores = {};
...
function applyDoctrine(snapshot, doctrine, factor) {
...
function applyStateModifiers(snapshot, state, factionId, difficulty) {
    ...
    const threatenedCities = cityIds.filter((cityId) => {
        ...
        if (!city)
            return false;
        return Array.from(state.units.values()).some((unit) => unit.hp > 0 && unit.factionId !== factionId && hexDistance(unit.position, city.position) <= 3);
    }).length;
    if (exhaustion >= 8) {
    ...
    if (visibleEnemySkirmishers > 0 && profile.personality.antiSkirmishResponseWeight > 0) {
        const responsePressure = Math.min(3, visibleEnemySkirmishers * profile.personality.antiSkirmishResponseWeight * 0.25);
...
function createBaseSnapshot(state, factionId, baseline) {
    return {
        factionId,
        round: state.round,
        scalars: {
...
function applyDifficultyBaselineOverrides(baseline, difficulty) {
    const profile = getAiDifficultyProfile(difficulty);
...
export function createEmptyAiPersonalitySnapshot(factionId, round = 0) {
    return {
        ...createBaseSnapshot({ round }, factionId, DEFAULT_BASELINE),
        round,
        reasons: ['baseline=default'],
    };
}
export function computeAiPersonalitySnapshot(state, factionId, registry, difficulty) {
    const profile = getAiDifficultyProfile(difficulty);
    const faction = state.factions.get(factionId);
    const baseline = applyDifficultyBaselineOverrides(registry.getFactionAiBaseline(factionId) ?? DEFAULT_BASELINE, difficulty);
...
export function scorePosture(snapshot, context, posture) {
...
export function scoreFocusTarget(snapshot, context, target) {
    let score = 0;
    if (target.isolated)
        score += snapshot.targetWeights.isolated ?? 0;
    if (target.capturable)
        score += snapshot.targetWeights.capturable ?? 0;
    if (target.wounded)
        score += snapshot.targetWeights.wounded ?? 0;
    if (target.isCity)
        score += snapshot.targetWeights.city ?? 0;
    if (target.terrainId)
        score += snapshot.terrainScores[target.terrainId] ?? 0;
    score -= (context.targetDistance ?? 0) * 0.1;
    return score;
}
export function scoreProductionCandidate(snapshot, context, prototype) {
...
export function scoreResearchCandidate(snapshot, _context, node) {
    let score = 0;
    if (node.domainId)
        score += snapshot.researchWeights[node.domainId] ?? 0;
    for (const codified of node.codifies ?? []) {
        score += snapshot.researchWeights[codified] ?? 0;
    }
    return score;
}
export function shouldCommitAttack(snapshot, context) {
    return (context.attackAdvantage ?? 0) >= snapshot.thresholds.commitAdvantage;
}
export function shouldRetreat(snapshot, context) {
    return (context.retreatRisk ?? 0) >= snapshot.thresholds.retreatThreshold;
}
export function getTerrainFitScore(snapshot, state, hex) {
