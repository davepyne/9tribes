 ...
 */
export function collectCombatSignals(attackerTerrain, defenderTerrain, attackerRole, attackerWeaponTags, defenderMovementClass, attackerTags) {
 ...
 */
export function applyCombatSignals(state, factionId, signals) {
    let current = state;
    for (const signal of signals) {
        const mappings = SIGNAL_CAPABILITY_MAP.filter((m) => m.signal === signal);
        for (const mapping of mappings) {
            current = addCapabilityProgress(current, factionId, mapping.domain, mapping.amount, 'combat', `${signal} experience`);
        }
    }
    return current;
}
