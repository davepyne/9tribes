...
import { assertSettlementOwnershipConsistency, getFactionCityCount, getFactionVillageCount, } from './factionOwnershipSystem.js';
...
import { createSimulationTrace, getVictoryStatus, runWarEcologySimulation, } from './warEcologySimulation.js';
export const SMOKE_HARNESS_SEEDS = [11, 23, 37, 41, 59, 73, 89, 97, 101, 131];
...
export const STRATIFIED_HARNESS_SEEDS = Object.values(STRATIFIED_HARNESS_SEEDS_BY_ARCHETYPE).flat();
export const DEFAULT_HARNESS_TURNS = 150;
function roundMetric(value) {
    return Number(value.toFixed(3));
}
function countTraceEvents(trace) {
    return {
...
function classifyMapArchetype(state) {
    const counts = Array.from(state.map?.tiles.values() ?? []).reduce((acc, tile) => {
...
function countSignatureUnits(state, factionId) {
...
function countSignatureLikePrototype(factionId, prototype) {
    const tags = new Set(prototype.tags ?? []);
    switch (factionId) {
        case 'jungle_clan':
...
function isSignatureCapableUnit(factionId, prototype, registry) {
    const tags = new Set(prototype.tags ?? []);
    const ability = registry.getSignatureAbility(factionId);
    if (ability?.stampedeBonus && (prototype.chassisId === 'elephant_frame' || tags.has('elephant')))
        return true;
    if (ability?.hitAndRun && prototype.derivedStats.role === 'mounted')
        return true;
    if (ability?.tidalAssaultBonus && (tags.has('naval') || tags.has('transport')))
        return true;
    if (ability?.venomDamagePerTurn && tags.has('poison'))
        return true;
    if (ability?.forestHealRate && (tags.has('druid') || tags.has('healing')))
        return true;
    if (ability?.bulwarkDefenseBonus && tags.has('fortress'))
        return true;
    if (ability?.desertSwarmAttackBonus && (tags.has('camel') || tags.has('desert')))
        return true;
    if (ability?.swiftChargeBonus && (prototype.derivedStats.role === 'mounted' || tags.has('river') || tags.has('amphibious')))
        return true;
    if (ability?.wallDefenseMultiplier && tags.has('transport'))
        return true;
    if (ability?.greedyBonus && tags.has('capture'))
        return true;
    if (ability?.summon && tags.has('cold'))
        return true;
    return countSignatureLikePrototype(factionId, prototype) > 0;
}
function getPrototypeProductionCost(prototype, faction) {
...
function averageRecordCounts(records) {
    const totals = {};
...
function getStalledProductionMetrics(state, factionId) {
    return Array.from(state.cities.values())
        .filter((city) => city.factionId === factionId && city.currentProduction)
        .map((city) => {
        const estimatedTurnsQueued = city.currentProduction?.costType === 'production'
...
function getFactionMetrics(state, factionId, registry) {
    const faction = state.factions.get(factionId);
    if (!faction) {
        return {
            factionId,
    ...
    const livingUnits = faction.unitIds
        ...
        .filter((unit) => Boolean(unit && unit.hp > 0));
    ...
    const supplyUtilizationRatio = economy.supplyIncome > 0
        ...
        : 0;
    const unitsByPrototypeId = livingUnits.reduce((acc, unit) => {
    ...
    const fieldedPrototypeCosts = livingUnits
        ...
        .filter((prototype) => Boolean(prototype))
...
function collectUnitComposition(state, livingUnits) {
    const byChassis = {};
    const byRole = {};
...
function averageUnitComposition(compositions) {
    const byChassis = {};
    const byRole = {};
    ...
    for (const key of Object.keys(byChassis))
        byChassis[key] = roundMetric(byChassis[key] * 10) / 10;
    for (const key of Object.keys(byRole))
        byRole[key] = roundMetric(byRole[key] * 10) / 10;
    return { byChassis, byRole };
}
export function collectSeedBalanceMetrics(seed, registry, maxTurns = DEFAULT_HARNESS_TURNS, mapMode = 'fixed', balanceOverrides, difficulty) {
    ...
    const routedUnits = livingUnits.filter((unit) => unit.routed).length;
    const factions = Object.fromEntries(factionConfigs.map((config) => {
        const factionId = config.id;
    ...
    return {
        ...
        livingUnits: livingUnits.length,
        routedUnits,
        totalWarExhaustion: roundMetric(Array.from(finalState.warExhaustion.values())
...
export function runBalanceHarness(registry, seeds = SMOKE_HARNESS_SEEDS, maxTurns = DEFAULT_HARNESS_TURNS, mapMode = 'fixed', balanceOverrides, difficulty) {
    const factionConfigs = getMvpFactionConfigs(balanceOverrides);
    const runs = seeds.map((seed) => collectSeedBalanceMetrics(seed, registry, maxTurns, mapMode, balanceOverrides, difficulty));
    ...
    const factions = Object.fromEntries(factionIds.map((factionId) => {
        ...
        return [
            factionId,
            {
                ...
                avgSignatureCapableUnits: roundMetric(factionRuns.reduce((sum, run) => sum + run.signatureCapableUnits, 0) / runs.length),
                ...
                avgSupplyUtilizationRatio: roundMetric(factionRuns.reduce((sum, run) => sum + run.supplyUtilizationRatio, 0) / runs.length),
                avgHighestAvailableProductionCost: roundMetric(factionRuns.reduce((sum, run) => sum + run.highestAvailableProductionCost, 0) / runs.length),
                avgHighestFieldedProductionCost: roundMetric(factionRuns.reduce((sum, run) => sum + run.highestFieldedProductionCost, 0) / runs.length),
                avgAverageFieldedProductionCost: roundMetric(factionRuns.reduce((sum, run) => sum + run.averageFieldedProductionCost, 0) / runs.length),
                avgUnitsByPrototypeId: averageRecordCounts(factionRuns.map((run) => run.unitsByPrototypeId)),
                avgStalledProductionCount: roundMetric(factionRuns.reduce((sum, run) => sum + run.stalledProduction.length, 0) / runs.length),
                avgUnitComposition: averageUnitComposition(factionRuns.map((run) => run.unitComposition)),
            },
        ];
    }));
    const mapArchetypes = runs.reduce((acc, run) => {
    ...
    return {
        ...
        avgTotalWarExhaustion: roundMetric(runs.reduce((sum, run) => sum + run.totalWarExhaustion, 0) / runs.length),
...
export function runStratifiedBalanceHarness(registry, maxTurns = DEFAULT_HARNESS_TURNS, mapMode = 'fixed', balanceOverrides, difficulty) {
    return runBalanceHarness(registry, STRATIFIED_HARNESS_SEEDS, maxTurns, mapMode, balanceOverrides, difficulty);
}
function getComparisonMetrics(metrics) {
    const avgFieldingGap = roundMetric(metrics.avgHighestAvailableProductionCost - metrics.avgHighestFieldedProductionCost);
    return {
...
function diffComparisonMetrics(hardMetrics, normalMetrics) {
    return {
        avgSupplyUtilizationRatio: roundMetric(hardMetrics.avgSupplyUtilizationRatio - normalMetrics.avgSupplyUtilizationRatio),
        avgHighestAvailableProductionCost: roundMetric(hardMetrics.avgHighestAvailableProductionCost - normalMetrics.avgHighestAvailableProductionCost),
        avgHighestFieldedProductionCost: roundMetric(hardMetrics.avgHighestFieldedProductionCost - normalMetrics.avgHighestFieldedProductionCost),
        ...
        avgSignatureCapableUnits: roundMetric(hardMetrics.avgSignatureCapableUnits - normalMetrics.avgSignatureCapableUnits),
        avgStalledProductionCount: roundMetric(hardMetrics.avgStalledProductionCount - normalMetrics.avgStalledProductionCount),
    };
}
export function runPairedDifficultyBalanceHarness(registry, seeds = SMOKE_HARNESS_SEEDS, maxTurns = DEFAULT_HARNESS_TURNS, mapMode = 'fixed', balanceOverrides) {
...
export function runStratifiedPairedDifficultyBalanceHarness(registry, maxTurns = DEFAULT_HARNESS_TURNS, mapMode = 'fixed', balanceOverrides) {
