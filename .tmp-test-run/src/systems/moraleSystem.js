 ...
 */
export function calculateMoraleLoss(damageTaken, maxHp, veteranMoraleBonus) {
    if (damageTaken <= 0)
        return 0;
 ...
 */
export function applyMoraleLoss(unit, loss) {
 ...
 */
export function recoverMorale(unit) {
    if (unit.hp <= 0)
        return unit.morale;
 ...
 */
export function checkRally(unit) {
    if (!unit.routed)
        return false;
 ...
 */
export function findFleeHex(unit, state) {
    ...
    for (const [, other] of state.units) {
        if (other.factionId === unit.factionId || other.hp <= 0)
            continue;
        if (!isUnitVisibleTo(state, unit.factionId, other))
            continue;
