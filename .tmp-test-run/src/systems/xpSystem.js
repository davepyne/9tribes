...
const VETERAN_LEVEL_ORDER = ['green', 'seasoned', 'veteran', 'elite'];
/**
 * Award XP to a unit after combat.
 * Returns a new Unit object with updated XP (immutable).
 */
export function awardCombatXP(unit, killedEnemy, survived) {
 ...
 */
export function getNextVeteranLevel(currentLevel) {
 ...
 * Returns null if already at max level.
 */
export function getXPForNextLevel(currentLevel, registry) {
 ...
 */
export function canPromote(unit, registry) {
 ...
 * Returns null if already at max level.
 */
export function promoteUnit(unit, registry) {
