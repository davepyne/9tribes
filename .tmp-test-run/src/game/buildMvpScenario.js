...
import { createFactionId, createUnitId, createCityId, createImprovementId } from '../core/ids.js';
...
const MAP_SIZE_DIMENSIONS = {
    small: { width: 40, height: 30 },
    medium: { width: 50, height: 38 },
    large: { width: 60, height: 46 },
};
function getClimateBandStartSeparation(width, height, factionCount) {
    if (factionCount <= 0) {
        return 10;
    }
...
function stampTerrainPatch(state, center, terrain, radius = 2) {
    const map = state.map;
    if (!map) {
        return;
    }
 ...
 */
export function buildMvpScenario(seed, options = {}) {
    ...
    const factionConfigs = getMvpFactionConfigs(options.balanceOverrides).filter((config) => requestedFactionIds.size === 0 || requestedFactionIds.has(config.id));
    ...
    if (mapMode === 'randomClimateBands') {
        const generated = generateClimateBandMap(state.rngState, factionConfigs.map((config) => ({
            factionId: config.id,
            homeBiome: config.homeBiome,
            ...
            mode: mapMode,
            startSeparation: getClimateBandStartSeparation(mapDimensions?.width ?? scenarioConfig.mapWidth, mapDimensions?.height ?? scenarioConfig.mapHeight, factionConfigs.length),
        ...
        state.map = generateMvpMap(state.rngState, mapDimensions?.width ?? scenarioConfig.mapWidth, mapDimensions?.height ?? scenarioConfig.mapHeight);
    ...
    for (let i = 0; i < factionConfigs.length; i++) {
        ...
        const settlerPrototype = assemblePrototype(factionId, 'infantry_frame', ['basic_spear', 'simple_armor'], registry, existingPrototypeIds, {
            faction,
            validation: {
        ...
        for (const unitConfig of unitConfigs) {
            ...
            // Assemble prototype
            const prototype = assemblePrototype(factionId, unitConfig.chassisId, unitConfig.componentIds, registry, existingPrototypeIds, {
                faction,
                validation: {
