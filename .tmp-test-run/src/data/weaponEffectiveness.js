...
export const weaponEffectivenessTable = [
...
export function getWeaponEffectiveness(weaponTags, targetMovementClass) {
    let totalModifier = 0;
    for (const tag of weaponTags) {
        const entry = weaponEffectivenessTable.find((e) => e.weaponTag === tag && e.targetMovementClass === targetMovementClass);
        if (entry) {
