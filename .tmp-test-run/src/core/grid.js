...
export const getNeighbors = (coord) => ISO_DIRECTIONS.map(dir => ({
    q: coord.q + dir.q,
    r: coord.r + dir.r,
}));
export const getDirectionVector = (direction) => ISO_DIRECTIONS[((direction % DIRECTION_COUNT) + DIRECTION_COUNT) % DIRECTION_COUNT];
...
export const getOppositeDirection = (direction) => (direction + 4) % DIRECTION_COUNT;
/**
 * Chebyshev distance — matches movement cost semantics:
 * adjacent tiles are distance 1, diagonals are also distance 1.
 */
export const tileDistance = (a, b) => Math.max(Math.abs(a.q - b.q), Math.abs(a.r - b.r));
/** Convert tile coordinate to string key */
export const tileToKey = (coord) => `${coord.q},${coord.r}`;
